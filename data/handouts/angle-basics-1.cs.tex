...
\EnvId{E7dY7LX9YobFdS39tfDDR-vrcholove-souhlasne-stridave-uhly}
\Exercise{}{
    Uvědomte si, že:
    \begitems \style a
...
\EnvId{G2fX3226MoyMyLEg7bCt3-soucet-uhlu-v-trojuhelniku}
\Theorem{}{
...
\EnvId{H7yQvLt3VxyGlELBTgAJh-soucet-uhlu-konvexniho-mnohouhelniku}
\Theorem{}{
...
\EnvId{vUBGcz0IXvF69ZPtrH_4F-soucet-uhlu-na-obrazku}
\Exercise{}{
...
\EnvId{EMfGZzO5Ro3VypJ7caLES-uhel-s-otaznikem-na-obrazku}
\Exercise{}{
...
\EnvId{YVYLXXXSL6OFFUuABkNDD-veta-sss-o-shodnosti}
\Example{}{
...
\EnvId{vMyC0sIAGJZq-NeOnkTWW-veta-sus-o-shodnosti}
\Exercise{}{
...
\EnvId{PmP9xfoxbBUEnYfG89CCT-veta-usu-o-shodnosti}
\Exercise{}{
...
\EnvId{G0TTdrAZrHu-Oip2mtDlP-shodnost-ssu}
...
\EnvId{GMR2XuASqafLT4gCEpuRI-rovnoramenny-trojuhelnik-shodne-uhly}
...
\EnvId{rKssiyhl1D8AnznvYHFaN-uhly-rovnostranneho-trojuhelniku}
\Exercise{}{
...
\EnvId{Dh7EmG5yYRQD44zhd5YD6-rovnoramenny-pravouhly-trojuhelnik}
\Exercise{}{
...
\EnvId{PVssK3ww1Ww4_Lm3f4PrT-pravouhly-trojuhelnik-dvojnasobna-prepona}
\Problem{0}{}{
...
\EnvId{kTzIqbV0OaJSgPvurjL5y-osa-usecky}
\Theorem{osa úsečky}{
    Množina bodů, které mají stejnou vzdálenost od krajních bodů úsečky $AB$, je kolmice procházející jejím středem~$M$.
}{
    \textit{($\Rightarrow$)} Nechť bod~$X$ leží na kolmici na $AB$ vedené středem~$M$. Trojúhelníky $AMX$ a~$BMX$ jsou shodné podle věty $sus$: $|MA|=|MB|$ (střed), společná strana $MX$ a~shodné pravé úhly při~$M$. Odtud $|XA|=|XB|$.

    \Image{angles-perpendicular-bisector-forward.pdf}

    \textit{($\Leftarrow$)} Předpokládejme, že bod~$X$ splňuje $|XA|=|XB|$, ale neleží na kolmici na $AB$ procházející~$M$. Pokud $X$ leží na přímce $AB$, je zřejmé, že jediný bod této přímky stejně vzdálený od~$A$ i~$B$ je právě střed~$M$.
...
\EnvId{FNKSPjq6yM0S5N_PL53GL-existence-stredu-kruznice-opsane}
\Theorem{existence~$O$}{
    ...
    \Remark{1} Bod $O$ splňuje $|OA|=|OB|=|OC|$, takže je středem kružnice procházející všemi třemi vrcholy -- tedy kružnice opsané trojúhelníku $ABC$.

    \Remark{2} Označení $O$ pro střed opsané kružnice je mezinárodní standard. V~Česku a~na Slovensku se rovněž používá~$S$ (ze slova \uv{střed}).
}

\secc Osa úhlu

Podobně jako u~osy strany se nejprve zamyslíme nad definicí. Mějme úhel $XAY$. Pod jeho \textit{osou} (přesněji \textit{vnitřní osou}) rozumíme polopřímku~$AZ$ takovou, že platí $|\angle XAZ|=|\angle ZAY|$. Bod~$Z$ je tedy zvolen tak, že polopřímka~$AZ$ rozdělí úhel $XAY$ na dva shodné úhly. Volně se za osu úhlu pokládá i~celá přímka určená touto polopřímkou.
...
\EnvId{PoEX7OH-T0op5IPL9DEhz-osa-uhlu}
\Theorem{osa úhlu}{
    Bod~$Z$ uvnitř úhlu $XAY$ má stejnou vzdálenost od přímek $AX$ a~$AY$ právě tehdy, když $|\angle XAZ|=|\angle ZAY|$.
}{
    \textit{($\Rightarrow$)} Nechť polopřímka~$AZ$ dělí úhel $XAY$ na dva shodné úhly. Označme $X'$, $Y'$ paty kolmic spuštěných z~$Z$ na přímky $AX$, $AY$. Pravoúhlé trojúhelníky $AX'Z$ a~$AY'Z$ jsou shodné podle věty~$usu$: společná přepona $AZ$, shodné úhly při~$A$ a~shodné pravé úhly při~$X'$, $Y'$. Odtud $|ZX'|=|ZY'|$, což jsou právě vzdálenosti bodu~$Z$ od přímek $AX$ a~$AY$.

    \Image{angles-bisector-equidistant.pdf}

    \textit{($\Leftarrow$)} Nechť naopak $|ZX'|=|ZY'|$, kde $X'$, $Y'$ jsou opět paty kolmic z~$Z$ na přímky $AX$, $AY$. Pravoúhlé trojúhelníky $AX'Z$ a~$AY'Z$ jsou shodné podle věty~$Ssu$: společná přepona $AZ$, shodné odvěsny $|ZX'|=|ZY'|$ a~shodné pravé úhly při~$X'$, $Y'$ naproti této přeponě (nejdelší straně). Odtud $|\angle X'AZ|=|\angle Y'AZ|$, což je přesně $|\angle XAZ|=|\angle ZAY|$.
}

Než si tuto větu naplno užijeme, zaveďme ještě pojem \textit{vnější osy úhlu}. Vnější osou úhlu $XAY$ rozumíme sjednocení os dvou úhlů vedlejších k~$XAY$ -- označme si je $XAY'$ a~$X'AY$, kde $X'$, $Y'$ leží na opačných polopřímkách k~$AX$, $AY$. Osy těchto dvou vedlejších úhlů jsou opačné polopřímky vycházející z~bodu $A$; jejich sjednocení je tedy přímka procházející~$A$.

\Image{angles-bisector-external-definition.pdf}

Než půjdeme dál, dokažme si tuto základní vlastnost:

\EnvId{g42rGrZiZDuBQcCBvCKh8-vnitrni-a-vnejsi-osa-uhlu}
\Exercise{}{
...
\EnvId{lHvoHwA3z301D2go6rtd5-existence-stredu-kruznice-vepsane}
\Theorem{existence~$I$}{
    ...
    \Remark{1} Bod~$I$ má stejnou vzdálenost od všech tří stran, takže je středem kružnice, která se dotýká všech tří stran zevnitř -- tedy kružnice vepsané trojúhelníku $ABC$.

    \Remark{2} Označení~$I$ pochází z~anglického slova \uv{incentrum}. V~česko-slovenské olympiádě se často používá prostě~$S$ (jako střed).
}

V~úlohách se velmi často objeví i~\textit{kružnice připsané}.

\EnvId{Mm527eX2RrorSGRJEbga4-existence-stredu-kruznice-pripsane}
\Theorem{existence~$I_a$}{
    ...
    \Remark{1} Bod~$I_a$ má stejnou vzdálenost od přímky $BC$ i~od přímek obsahujících strany $AB$, $AC$, takže je středem kružnice dotýkající se strany $BC$ zvenku a~zbylých dvou stran (přesněji jejich prodloužení) zevnitř -- kružnice připsané trojúhelníku proti vrcholu~$A$.

    \Remark{2} Označení~$I_a$ je běžné mezinárodní značení. Kružnici připsané se říká \uv{excircle}, takže možná logičtější by bylo $E_a$, ale to se spíše nepoužívá. Kružnice vepsané ale souvisí s~incentrem, takže možná proto \UpsideDownSmile.
}
...
\EnvId{Vu5msR8YT8yuZ3zb8NeRJ-shodne-usecky-v-trojuhelniku}
...
\EnvId{SaOp1yOLr0T9xO-XywvHv-rovnostranne-trojuhelniky-ve-ctverci}
...
\EnvId{q5FKHnEWB5kfNu_ZGuq49-osy-uhlu-v-lichobezniku}
...
\EnvId{QyZrP8_I1-CPXFt7t7zed-rovnobeznost-v-konvexnim-petiuhelniku}
...
\EnvId{my3-6U8Mr3piG2sLobH4A-delka-usecky-ve-ctyruhelniku}
\Problem{0}{DuoGeo 2025}{
