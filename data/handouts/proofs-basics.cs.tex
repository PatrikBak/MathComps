...
\EnvId{6KrSLRVFvoy9OdoVfehCQ-ctverce-lichych-cisel-modulo-8}
\Example{}{
...
\EnvId{sftFVZqBaqREN0z-UcX-t-ctverec-prvocisla-minus-jedna}
\Exercise{}{
    ...
    Pro zajímavost uveďme ještě jedno řešení, které používá poznatek z~předešlého příkladu: čtverce lichých čísel dávají po dělení~8 zbytek~1: Díky tomu vlastně máme, že $p^2-1$ je dělitelné~8 -- zbývá zdůvodnit, že $p^2-1$ je dělitelné~3. Jelikož $p$ není~3, tak $p$ je ve tvaru buď $3k+1$ nebo $3k-1$ pro nějaké~$k$, ve zkratce $p=3k \pm 1$. Potom 
    $$
...
\EnvId{x5mL1lT3zcfYgisxPSA-W-lichy-pocet-delitelu-ctverce}
\Example{}{
...
\EnvId{Kk7dZfMi6QJfF9r1Bwa8B-veta-o-poctu-delitelu}
\Theorem{}{
    ...
    Uvažujme libovolného dělitele $d$ čísla $n$. Jelikož $d$ dělí $n$ a~prvočíselný rozklad $n$ obsahuje pouze prvočísla $p_1,\dots,p_k$, i rozklad čísla $d$ musí obsahovat pouze tato prvočísla. Zapišme tedy $d = p_1^{b_1} \cdot p_2^{b_2} \cdots p_k^{b_k}$. Přitom pro každé $i$ musí platit $0 \leq b_i \leq a_i$ (horní hranice plyne z~toho, že $d$ jako dělitel $n$ by nemohl obsahovat vyšší mocninu $p_i$ než $n$). Každého dělitele tedy umíme jednoznačně reprezentovat jako $k$-tici $(b_1,\dots,b_k)$ s~vlastností $0 \leq b_i \leq a_i$. Počet dělitelů je proto roven počtu takovýchto $k$-tic.

    Na určení počtu $k$-tic použijeme kombinatorické pravidlo součinu. Volby jednotlivých exponentů $b_1,\dots,b_k$ jsou na sobě nezávislé -- hodnota $b_i$ nijak neomezuje hodnoty ostatních exponentů. Pro exponent $b_i$ máme $a_i+1$ možností, konkrétně všechna čísla od $0$ do $a_i$. Podle pravidla součinu je celkový počet $k$-tic roven
    $$
    (a_1+1)(a_2+1)\cdots (a_k+1).
    $$
}

\EnvId{o4FSeK3jvHdTEakt1JnAY-pocet-delitelu-a-obracena-implikace}
\Exercise{}{
...
\EnvId{K8Jhyizt5YvLRX463Ka_e-prvocislo-nepresahujici-odmocninu-n}
\Problem{0}{}{
...
\EnvId{C0sb6mdImA-f9i_DF9ajq-soucin-vsech-delitelu-cisla}
\Problem{1}{}{
...
Na závěr sekce si jedno upozornění. Při sepisování důkazů je často vidět nepřesnost typu \textit{vynechání předpokladů}, například někdo může napsat \uv{$x^2=1$, takže nutně $x=1$}. To může být správná část řešení, pokud v~zadání máme, že $x$ je kladné. Pokud však tento předpoklad do řešení nenapíšeme, tak to zní podezřele, protože není jasné, zda si to řešitel uvědomil, a~proto to může vést ke ztrátě bodů. Správně bychom psali něco jako \uv{$x^2=1$ a $x$ je kladné, takže nutně $x=1$}. Ukážeme si velmi běžnou situaci, kdy se toto stává:

\EnvId{YXqKtQowN8gPVViw4PWN4-vzajemna-delitelnost-dvou-cisel}
\Example{}{
	Kdy pro celá čísla $a,b$ platí, že $a \mid b$ a $b \mid a$?
}{
	\Answer{$a=b$ nebo $a=-b$.}
	
	Intuitivní odpověď je, že nutně $a=b$. V~řešení občas opravdu vidět úvahu, že $a \mid b$ a $b \mid a$, takže $a=b$. To opravdu platí, pokud víme, že čísla $a,b$ jsou nezáporná. Situaci si zanalyzujeme podrobně:
	
	Vztah $a \mid b$ dává, že existuje celé $u$ takové, že $b=au$. Podobně $b \mid a$ znamená, že existuje celé $v$ takové, že $a=bv$. Vynásobením těchto rovností máme $ab=abuv$. 
	
	Pokud $ab=0$, tedy pokud např. $a=0$, tak vztah $b=au$ dává $b=0$. Potom oba vztahy znamenají $0 \mid 0$, což je v~pořádku\fnote{Na první pohled může být překvapivé, že $0 \mid 0$, přece jen, nulou se nedělí. Dělitelnost se ale definuje tak, že $x \mid y$ právě když existuje celé $z$ takové, že $y=zx$. Pro $x=y=0$ takové $z$ existuje, dokonce vyhovuje každé celé $z$} a~platí $a=b$.
	
	Pokud $ab\neq 0$, tak $ab=abuv$ dává $1=uv$. Číslo~$1$ se dá zapsat jako součin celých čísel právě dvěma způsoby: $1 \cdot 1$ a $(-1) \cdot (-1)$. To nám dává dvě řešení $(u,v)=(1,1)$ a $(u,v)=(-1,-1)$, která zpětně znamenají $a=b$ a $a=-b$. Pokud bychom však měli, že $a,b$ jsou nezáporná, tak buď je nějaké nulové, to bylo vyřešeno výše, nebo jsou obě kladná, což ale při $a=-b$ nenastane -- tehdy bychom tedy opravdu měli, že nutně $a=b$. 
	
	Závěr je, že $a \mid b$ a $b \mid a$ právě tehdy, když $a=b$ nebo $a=-b$, přičemž pro nezáporná čísla dokonce rovnou $a=b$.
}
...
\EnvId{pPJ8A14vh3r-vsHx4AMP1-iracionalita-odmocniny-ze-dvou}
\Example{}{
...
\EnvId{64NL2y_73vGHKsm_iuBXS-selhani-dukazu-pro-odmocninu-4}
\Exercise{}{
...
\EnvId{MygIexYlJA93Zs1GKPipd-kriterium-iracionality-odmocniny-n}
\Exercise{}{
...
\EnvId{SselxmWyMFZXmodhe5HKW-soucet-racionalniho-a-iracionalniho-cisla}
\Problem{0}{}{
...
\EnvId{lEtf_CHzL0kz2p9TGK5Ml-odmocnina-2-plus-odmocnina-3}
\Problem{0}{}{
...
\EnvId{y5MK6awUXD1Q21xcYhP-r-soucet-tri-lichych-ctvercu}
\Problem{0}{}{
...
\EnvId{YFmWOBiEPkcdH8w_XCCXq-karty-s-cisly-na-kruhu}
\Problem{0}{73-CSMO-B-II-1}{
	Patrik vybral dvě různá kladná celá čísla $a$, $b$, každé napsal na 10 karet a všech 20\,karet rozmístil po obvodu kruhu. Všiml si, že každé číslo je nyní dělitelem součtu dvou čísel na sousedních kartách. Dokažte, že čísla na kartách se ob jedno střídají.
}{
	...
	Kdyby se čísla nestřídala, tak nějaká dvě stejná jsou vedle sebe, např. $aa$. Když se nyní procházíme po kružnici nějakým směrem, např. doprava, tak po čase musíme narazit na $b$, takže máme $aab$. Co z toho dostaneme?
}{
	Trojice $aab$ nám dává $a \mid a+b$, takže $a \mid b$. Bylo by skvělé najít něco podobného ještě někde jinde.
}{
	Abychom dostali $b \mid a$, potřebujeme najít trojici $bba$. Proč na kružnici musí existovat i $bb$? Zamysli se nad počtem střídavých skupin: pokud spojíš dvě $a$ do jedné skupiny, snížíš tím počet dostupných skupin pro $b$. Kam se potom musí vejít všechna $b$?
}{
	Pokud z trojice $aab$ zjistíme, že $a \mid b$, a z trojice $bba$ zjistíme, že $b \mid a$, jaký závěr z toho vyplývá pro dvě přirozená čísla $a, b$?
}{
...
\EnvId{KvlppDtT0_6whJQhO-l8K-nekonecne-mnoho-prvocisel}
\Theorem{}{
...
\EnvId{xcyxtxvzStwwMDpYr7pv2-prvocisla-delici-n2-plus-1}
\Problem{1}{}{
...
\EnvId{f8Ikrt3vc35M1w9ez9TsY-prvocisla-tvaru-3n-plus-2}
\Problem{2}{}{
    ...
    Předpokládejme, že prvočísel tvaru $3n+2$ je jen konečně mnoho.
