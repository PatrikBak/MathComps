...
\Definition{Spirální podobnost}{
    \textit{Spirální podobnost} je geometrické zobrazení, které je složením otočení a~stejnolehlosti. Je určena středem, orientovaným úhlem otočení $\phi$ a~koeficientem stejnolehlosti $k>0$.
...
\EnvId{JmA51D6zLoKYiJEsuQBWn-spiralni-podobnost-po-dvou}
\Theorem{Spirální podobnost chodí \uv{po dvou}}{
    ...
    Nechť má spirální podobnost úhel $\phi$ a~koeficient $k$. Přímo z~definice je $\angle AOA' = \angle BOB' = \phi$ (orientované úhly) a~$|OA'| : |OA| = |OB'| : |OB| = k$.

    Přičtením úhlu $A'OB$ k~oběma stranám první rovnosti máme $\angle AOB = \angle A'OB'$ a~úpravou druhé $|OB| : |OA| = |OB'| : |OA'|$. Spirální podobnost se středem $O$, úhlem $\angle AOB$ a~koeficientem $|OB| : |OA|$ tedy zobrazuje $A$ na $B$ a~$A'$ na $B'$, čili $AA'$ na $BB'$.
}

\EnvId{7w5OPOXaDCNOVEgIpddYa-spiralni-podobnost-a-podobne-body}
\Theorem{Spirální podobnost přenáší \uv{podobné body}}{
    Nechť $S$ je spirální podobnost se středem $O$ převádějící $AB$ na $A'B'$. Nechť $X$ a~$X'$ jsou body na úsečkách $AB$ a~$A'B'$ takové, že $|AX|:|BX|=|A'X'|:|B'X'|$. Potom $S$ přenáší $X$ na $X'$, a~pokud $S$ není stejnolehlost a~$O$ není žádný z~bodů $A$, $B$, $X$, jsou trojúhelníky $OAA'$, $OXX'$, $OBB'$ navzájem podobné.

    \Image{spiral-similar-points.pdf}
}{
    Podobnost $S$ zobrazuje úsečku $AB$ na úsečku $A'B'$ a~zachovává poměry délek, takže $S(X)$ je bod úsečky $A'B'$, který ji dělí v~poměru $|AX| : |BX|$. Takový bod je jediný, proto $S(X) = X'$.

    Trojúhelníky $OAA'$, $OXX'$, $OBB'$ mají u~vrcholu $O$ tentýž úhel $\phi$ spirální podobnosti a~tentýž poměr stran $|OA'| : |OA| = |OX'| : |OX| = |OB'| : |OB| = k$, takže jsou podobné podle věty $sus$.
}

\EnvId{eAqIhVm0OVC2uVXDRxMpJ-stred-spiralni-podobnosti}
\Theorem{Střed spirální podobnosti}{
    Nechť $A,B,C,D$ jsou body v~rovině takové, že $AB \nparallel CD$. Nechť se přímky $AB$ a~$CD$ protínají v~$X$, který je různý od $A$, $B$, $C$, $D$. Nechť se kružnice opsané trojúhelníkům $XAC$ a~$XBD$ protínají znovu v~$O$; pokud se v~$X$ dotýkají, položme $O = X$. Potom je $O$ středem jediné spirální podobnosti, která zobrazuje $AB$ na $CD$.

    \Image{spiral-center.pdf}
}{
    Počítáme s~orientovanými úhly přímek modulo $180^\circ$. Z~kružnice $XAOC$ je $\angle(AO, AB) = \angle(AO, AX) = \angle(CO, CX) = \angle(CO, CD)$ a~stejně tak z~kružnice $XBOD$ je $\angle(BO, BA) = \angle(DO, DC)$, takže trojúhelníky $OAB$ a~$OCD$ jsou souhlasně podobné. (Pokud se kružnice v~$X$ dotýkají, převádí jednu na druhou stejnolehlost se středem $X$; ta zobrazuje $A$ na $B$ a~$C$ na $D$, protože přímka $AB$ protíná první kružnici v~$X$ a~$A$ a~druhou v~$X$ a~$B$, a~podobně pro $CD$. Proto $|XB| : |XA| = |XD| : |XC|$ a~spirální podobnost se středem $X$ zobrazující $A$ na $C$ zobrazuje i~$B$ na $D$.)

    \Image{spiral-center-proof.pdf}

    Ze souhlasné podobnosti je $|OA| : |OC| = |OB| : |OD|$ a~rovnají se i~orientované úhly polopřímek $\angle AOB = \angle COD$, čili po přičtení úhlu $BOC$ také $\angle AOC = \angle BOD$. Podle věty $sus$ jsou proto souhlasně podobné i~trojúhelníky $OAC$ a~$OBD$, což přesně znamená, že spirální podobnost se středem $O$, úhlem $\angle AOC$ a~koeficientem $|OC| : |OA|$ zobrazuje $A$ na $C$ a~$B$ na $D$. Jiná spirální podobnost zobrazující $AB$ na $CD$ neexistuje: přímá podobnost je určena obrazy dvou bodů, protože každý další bod je určen vzdálenostmi od nich a~orientací.
}

Jelikož spirální podobnost chodí po dvou, tentýž bod $O$ je i~středem spirální podobnosti, která zobrazuje $AC$ na $BD$. Pokud navíc $AC \nparallel BD$, funguje stejný postup i~pro tuto dvojici: přímky $AC$ a~$BD$ se protnou v~bodě $Y$ a~$O$ je druhým průsečíkem kružnic opsaných trojúhelníkům $ABY$ a~$CDY$.
...
\i Pokud $AB \parallel CD$ a~$AC \nparallel BD$, první konstrukce selže (přímky $AB$ a~$CD$ se neprotínají), druhá však funguje: středem je $Y$ a~spirální podobnost zobrazující $AB$ na $CD$ je stejnolehlostí se středem $Y$, se záporným koeficientem, leží-li $Y$ mezi $A$ a~$C$, a~s~kladným, leží-li mimo úsečku $AC$. Kružnice $ABY$ a~$CDY$ se pak v~$Y$ dotýkají zvnějšku, resp. zevnitř.
   \Image{spiral-center-tangent.pdf}
   \Image{spiral-center-tangent-internal.pdf}
\i Pokud $AB \parallel CD$ a~zároveň $AC \parallel BD$, selžou obě konstrukce. Čtyřúhelník $ABDC$ je pak rovnoběžník a~zobrazení $A$ na $C$, $B$ na $D$ je posunutí, které střed nemá.
\enditems
...
\Theorem{Miquelův bod}{
    ...
    Přímky $AB$ a~$DC$ se protínají v~$Q$, takže podle věty o~středu spirální podobnosti je střed $M$ spirální podobnosti zobrazující $AB$ na $DC$ druhým průsečíkem kružnic $QAD$ a~$QBC$.

    Spirálka chodí po dvou, takže $M$ je i~středem spirální podobnosti zobrazující $AD$ na $BC$. Přímky $AD$ a~$BC$ se protínají v~$R$, a~tak tatáž věta dává, že $M$ je druhým průsečíkem kružnic $RAB$ a~$RDC$. Všechny čtyři kružnice tedy procházejí bodem $M$.
}
...
\Theorem{Miquelův bod tětivového čtyřúhelníku}{
    ...
    Počítáme s~orientovanými úhly přímek modulo $180^\circ$. Z~kružnic $QADM$ a~$RABM$, které dává předchozí věta, je
    $$
    \gather
    ...
    Bod $M$ leží na $QR$ právě tehdy, když přímky $MQ$ a~$MR$ splývají, tedy když $\angle(MQ, MA) = \angle(MR, MA)$. Podle vypsaných rovností to nastane právě tehdy, když $\angle(DC, DA) = \angle(BC, BA)$, a~to je přesně podmínka tětivovosti $ABCD$.
}

Označme ještě $P = AC \cap BD$. Tentýž postup pro zbývající dvě dvojice přímek dává dva další Miquelovy body:
\begitems
\i Kružnice opsané trojúhelníkům $PAB, PCD, QAC, QBD$ mají společný bod $M_1$, který je středem spirální podobnosti zobrazující $AB$ na $CD$, resp. $AC$ na $BD$. Tento bod leží na $PQ$ právě tehdy, když je $ABCD$ tětivový.
   \Image{spiral-miquel-others-pq.pdf}
\i Kružnice opsané trojúhelníkům $PAD, PCB, RAC, RDB$ mají společný bod $M_2$, který je středem spirální podobnosti zobrazující $AD$ na $CB$, resp. $AC$ na $DB$. Tento bod leží na $PR$ právě tehdy, když je $ABCD$ tětivový.
...
\Theorem{$OM \perp QR$}{
    ...
    Označme $X$ a~$Y$ středy úseček $AD$ a~$BC$. Bod $M$ je středem spirální podobnosti zobrazující $AD$ na $BC$ a~ta podle věty o~přenášení podobných bodů zobrazuje $X$ na $Y$, tedy i~úsečku $XD$ na $YC$. Přímky $XD = AD$ a~$YC = BC$ se protínají v~$R$, takže podle věty o~středu spirální podobnosti je $M$ druhým průsečíkem kružnic $RXY$ a~$RDC$.

    \Image{spiral-miquel-om-perpendicular-proof.pdf}

    Střed $O$ kružnice opsané leží na osách tětiv $AD$ a~$BC$, takže u~$X$ i~$Y$ je pravý úhel a~kružnice $RXY$ má průměr $OR$. Leží na ní i~$M$, a~proto $|\angle OMR| = 90^\circ$. Bod $M$ leží na $QR$ (předchozí věta), takže $OM \perp QR$.
}
...
\Theorem{$O$, $P$, $M$ na přímce}{
    ...
    Nejprve ukážeme, že $A$, $O$, $M$, $C$ leží na kružnici; počítáme s~orientovanými úhly přímek modulo $180^\circ$. Bod $M$ je středem spirální podobnosti zobrazující $A$ na $D$ a~$B$ na $C$, jejíž úhel je $\angle(AB, DC)$, a~zároveň spirální podobnosti zobrazující $A$ na $B$ a~$D$ na $C$ s~úhlem $\angle(AD, BC)$. Proto je $\angle(MA, MD) = \angle(AB, DC)$ a~zároveň $\angle(MD, MC) = \angle(AD, BC)$. Z~tětivovosti $ABCD$ je navíc $\angle(BC, DC) = \angle(AB, AD)$, a~tak
    $$
    \gather
    ...
    \endgather
    $$
    Podle věty o~obvodovém a~středovém úhlu je $2\angle(AB, BC) = \angle(OA, OC)$, takže $A$, $O$, $M$, $C$ leží na kružnici. Stejně tak leží na kružnici i~$B$, $O$, $M$, $D$. (Je-li $AC$ nebo $BD$ průměr, plyne z~téže rovnosti, že $M$ leží na tomto průměru, a~$O$, $P$, $M$ na něm leží přímo.)

    \Image{spiral-miquel-opm-line-proof.pdf}

    Obě kružnice procházejí body $O$ a~$M$, jejich chordálou je tedy přímka $OM$. Bod $P$ leží uvnitř tětiv $AC$ a~$BD$, tedy uvnitř obou kružnic, a~jeho mocnost vzhledem k~nim je $-|PA| \cdot |PC|$, resp. $-|PB| \cdot |PD|$; obě hodnoty se rovnají mocnosti $P$ vzhledem ke kružnici opsané $ABCD$. Bod $P$ tedy leží na chordále $OM$ a~kolmost $OP \perp QR$ plyne z~předchozí věty.
}
...
Ukážeme ještě jednu důležitou vlastnost z~projektivní geometrie, která souvisí s~polárami.

\EnvId{1LKVhRCTK_6pAfJ02sHWt-inverze-a-polara}
\Definition{Polára}{
    Nechť $\omega$ je kružnice se středem $O$ a~poloměrem $r$ a~nechť $P \neq O$ je bod. Obrazem bodu $P$ v~\textit{inverzi} vzhledem k~$\omega$ je bod $P'$ polopřímky $OP$, pro který platí $|OP| \cdot |OP'| = r^2$. \textit{Polára} bodu $P$ vzhledem k~$\omega$ je přímka procházející bodem $P'$ a~kolmá na $OP$.
}

\EnvId{8WFRq4Y9hX4Xf2REA_qfL-polara}
\Theorem{Polára}{
    Nechť $ABCD$ je tětivový čtyřúhelník. Označme $P=AC \cap BD$, $R=AD \cap BC$ a~$Q=AB \cap CD$. Potom $QR$ je polára bodu $P$ vzhledem ke kružnici opsané $ABCD$ se středem $O$, analogicky $PR$ je polára bodu $Q$ a~$PQ$ je polára bodu $R$.

    \Image{spiral-miquel-polar.pdf}
}{
    Označme $r$ poloměr kružnice opsané a~$M$ Miquelův bod na $QR$. Podle předchozí věty leží $O$, $P$, $M$ na přímce a~$A$, $O$, $M$, $C$ na kružnici (je-li $AC$ průměr, vezmeme kružnici $BOMD$ a~všude dále píšeme $BD$ místo $AC$); bod $P$ leží uvnitř tětivy $AC$, tedy uvnitř této kružnice, a~tak i~uvnitř její tětivy $OM$. Mocnost bodu $P$ vzhledem k~ní dává $|PO| \cdot |PM| = |PA| \cdot |PC| = r^2 - |OP|^2$, čili
    $$
    |OP| \cdot |OM| = |OP| \cdot \bigl(|OP| + |PM|\bigr) = r^2.
    $$
    Bod $M$ je tedy obrazem bodu $P$ v~inverzi vzhledem ke kružnici opsané.

    \Image{spiral-miquel-polar-proof.pdf}

    Polára bodu $P$ je proto přímka procházející bodem $M$ a~kolmá na $OM$. Bod $M$ leží na $QR$ a~$OM \perp QR$, takže touto přímkou je $QR$. Pro body $Q$ a~$R$ je úvaha analogická, jen ty leží vně kružnice opsané, takže jejich mocnost je kladná a~obraz v~inverzi padne mezi střed a~daný bod.
}

\sec Další známé konfigurace

\EnvId{tQ6r-YBHII2hYOcd0jnzC-ptolemaiova-veta}
\Theorem{Ptolemaiova věta}{
    Nechť $ABCD$ je konvexní čtyřúhelník se stranami $a=|AB|$, $b=|BC|$, $c=|CD|$, $d=|DA|$ a~úhlopříčkami $e=|AC|$, $f=|BD|$. Potom
    $$
    ac + bd \geq ef,
    $$
    ...
    Nechť $X$ je obraz bodu $D$ ve spirální podobnosti se středem $A$, která zobrazuje $C$ na $B$. Trojúhelníky $ACD$ a~$ABX$ jsou pak souhlasně podobné s~koeficientem $a/e$, takže $|BX| = ac/e$. Spirálka chodí po dvou, a~tak je $A$ i~středem spirální podobnosti zobrazující $CB$ na $DX$: trojúhelníky $ACB$ a~$ADX$ jsou souhlasně podobné s~koeficientem $d/e$ a~$|DX| = bd/e$.

    \Image{spiral-ptolemy-proof.pdf}

    Z~trojúhelníkové nerovnosti je
    $$
    f = |BD| \leq |BX| + |XD| = {ac + bd \over e},
    $$
    přičemž rovnost nastává právě tehdy, když $X$ leží na úsečce $BD$. Z~podobností je $|\angle ABX| = |\angle ACD|$ a~$|\angle ADX| = |\angle ACB|$, a~pro konvexní $ABCD$ jsou trojúhelníky $ABX$ a~$ABD$ souhlasně orientované a~stejně tak $ADX$ a~$ADB$. Bod $X$ tak leží na polopřímce $BD$ právě tehdy, když $|\angle ABD| = |\angle ACD|$, a~na polopřímce $DB$ právě tehdy, když $|\angle ADB| = |\angle ACB|$. Úsečka $BD$ je průnikem těchto dvou polopřímek a~každá z~rovností je ekvivalentní s~tětivovostí $ABCD$, takže rovnost nastává právě pro tětivový $ABCD$.
}

\EnvId{ECXlxtdJwECBnuRZW8E-V-simsonova-primka}
\Theorem{Simsonova přímka}{
    ...
    Označme $X$, $Y$, $Z$ paty kolmic z~bodu $D$ na přímky $AB$, $AC$, $BC$ a~počítejme s~orientovanými úhly přímek modulo $180^\circ$. Z~tětivovosti $ABCD$ plyne $\angle(BD, BX) = \angle(BD, BA) = \angle(CD, CA) = \angle(CD, CY)$ a~u~$X$ i~$Y$ je pravý úhel, takže pravoúhlé trojúhelníky $DXB$ a~$DYC$ jsou souhlasně podobné. Spirální podobnost $\sigma$ se středem $D$ zobrazující $B$ na $X$ proto zobrazuje $C$ na $Y$. (Pro $X = B$ je $AD$ průměr, pak je i~$Y = C$ a~všechny tři paty leží na $BC$.)

    \Image{spiral-simson-proof.pdf}

    Nechť $P$ je vzor bodu $Z$ v~$\sigma$. Trojúhelník $DZP$ je souhlasně podobný s~$DXB$, takže $|\angle DZP| = 90^\circ$ a~$P$ leží na přímce kolmé na $DZ$ v~bodě $Z$, což je přímka $BC$. Body $B$, $P$, $C$ tedy leží na přímce, a~tak na přímce leží i~jejich obrazy $X$, $Z$, $Y$.
}

\EnvId{J67DMaac8DHSjoZgc7hzh-sharky-devil}
\Theorem{Sharky-Devil}{
    Nechť se kružnice vepsaná trojúhelníku $ABC$ dotýká stran $BC$, $CA$, $AB$ po řadě v~bodech $D$, $E$, $F$ a~nechť se kružnice opsané trojúhelníkům $AEF$ a~$ABC$ protínají znovu v~bodě $S$ (pro $|AB| = |AC|$ se dotýkají v~$A$ a~tehdy je $S = A$). Potom přímka $SD$ prochází středem oblouku $BC$ neobsahujícího $A$, čili $SD$ je osou úhlu $BSC$.

    \Image{spiral-sharky-devil.pdf}
}{
    Přímky $FB$ a~$EC$ se protínají v~$A$ a~kružnice $AFE$ a~$ABC$ se protínají znovu v~$S$, takže podle věty o~středu spirální podobnosti je $S$ středem spirální podobnosti zobrazující $FB$ na $EC$. Její úhel je orientovaný úhel od polopřímky $FB$ k~polopřímce $EC$, čili orientovaný úhel $BAC$, a~rovná se orientovanému úhlu $BSC$; bod $S$ tedy vidí $BC$ pod stejným orientovaným úhlem jako $A$ a~leží na oblouku $BC$ obsahujícím $A$.

    Trojúhelníky $SFB$ a~$SEC$ jsou podobné a~tečny z~jednoho bodu jsou stejně dlouhé, $|BF| = |BD|$ a~$|CE| = |CD|$, takže
    $$
    {|SB| \over |SC|} = {|FB| \over |EC|} = {|DB| \over |DC|}.
    $$
    Podle obrácené věty o~ose úhlu je $SD$ osou úhlu $BSC$ a~osa obvodového úhlu $BSC$ prochází středem $N$ oblouku $BC$ neobsahujícího $S$, tedy neobsahujícího $A$.
}
...
\Theorem{}{
    ...
    Označme $N$ druhý průsečík kružnic $ADE$ a~$ABC$ (pro $|AB| = |AC|$ se dotýkají v~$A$ a~ten je pak sám středem oblouku $BAC$). Přímky $DB$ a~$EC$ se protínají v~$A$, takže podle věty o~středu spirální podobnosti je $N$ středem spirální podobnosti zobrazující $DB$ na $EC$. Její koeficient je $|EC| : |DB| = 1$, jde tedy o~otočení a~trojúhelníky $NDB$ a~$NEC$ jsou shodné. Speciálně $|NB| = |NC|$, čili $N$ je středem jednoho z~oblouků $BC$.

    \Image{spiral-arc-midpoint-proof.pdf}

    Úhel otočení je orientovaný úhel od polopřímky $DB$ k~polopřímce $EC$, čili orientovaný úhel $BAC$, a~rovná se orientovanému úhlu $BNC$. Bod $N$ tedy vidí $BC$ pod stejným orientovaným úhlem jako $A$ a~leží na oblouku $BC$ obsahujícím $A$.
}
...
}%
{
    ...
    Spirálka chodí po dvou, takže $A$ je i~středem spirální podobnosti $T$, která zobrazuje $BB'$ na $CC'$ a~$B \mapsto C$. Její úhel otočení je proto $|\angle BAC| = 90^\circ$, tedy $CC' = T(BB') \perp BB'$.
}
...
\Problem{0}{Česko-slovenská olympiáda 2024}{
    ...
    Spirálka chodí po dvou, a~tak ze zobrazení $BD$ na $KE$ máme, že $T$ je střed spirálky, která bere $BK$ na $DE$. Co to znamená v~řeči podobnosti?
}%
{
    \Answer{$|AT| : |DE| = 2\sqrt2$.}

    Trojúhelníky $TBK$ a~$TCL$ jsou pravoúhlé rovnoramenné s~pravým úhlem u~$K$, resp. $L$, a~jelikož $K$ a~$L$ leží v~polorovinách $BTC$ a~$CTA$ a~$T$ je vnitřní bod trojúhelníku $ABC$, jsou orientované souhlasně. Existuje tedy spirální podobnost se středem $T$ převádějící $BK$ na $CL$. Spirálka chodí po dvou, takže $T$ je i~středem spirální podobnosti $\sigma$, která převádí $BC$ na $KL$.

    \Image{spiral-centroid-isosceles.pdf}

    Podobnost zachovává středy úseček, proto $\sigma(D) = E$, a~jelikož spirálka chodí po dvou, je $T$ i~středem spirální podobnosti převádějící $BK$ na $DE$. Trojúhelníky $TBK$ a~$TDE$ jsou tak podobné, čili $TDE$ je pravoúhlý rovnoramenný s~přeponou $TD$ a~$|TD| = \sqrt2 \, |DE|$. Těžiště dělí těžnici $AD$ v~poměru $|AT| : |TD| = 2 : 1$, a~tak
    $$
    |AT| : |DE| = 2\sqrt2.
    $$
}
...
\Problem{0}{PraSe 27-3-8}{
    ...
    Kdykoli se dvě kružnice podezřele protínají v~jednom bodě, je velká šance, že je to Miquelův bod nějakého čtyřúhelníku.
}{
    Klíčem je, že $A$ je jeden z~Miquelů pro čtyřúhelník s~vrcholy $B,X,C,Y$. Tím pádem je středem různých spirálek. Ty se kamarádí se středy.
}{
    Klíčová spirálka je ta, která přenáší $XY$ na $BC$. Ta totiž přenáší i~středy na sebe. Její střed je tím pádem středem dalších dobrých spirálek. Trikem je uvědomit si, jak sestrojit její střed.
}{
    Poslední pozorování je, že $A$ je středem spirálky, která zobrazuje např. $BM$ na $XM'$. Aplikujeme algoritmus na nalezení středu, nejprve tyto úsečky protneme a~potom sestrojíme kružnice. Takto šikovně získáme tětivovost.
}%
{
    Přímky $XY$ a~$BC$ se protínají v~$D$, takže podle věty o~středu spirální podobnosti je druhý průsečík kružnic $(XBD) = (ABD)$ a~$(YCD) = (ACD)$, tedy bod $A$, středem spirální podobnosti zobrazující $XB$ na $YC$. Spirálka chodí po dvou, takže $A$ je i~středem spirální podobnosti $S$ zobrazující $XY$ na $BC$.

    Body $M'$ a~$M$ jsou středy úseček $XY$ a~$BC$, a~jelikož spirálka přenáší podobné body, $S(M') = M$. Podobnost $S$ tedy zobrazuje $XM'$ na $BM$, a~jelikož spirálka chodí po dvou, je $A$ středem spirální podobnosti zobrazující $XB$ na $M'M$.
...
\Problem{0}{}{
    Je dán ostroúhlý trojúhelník $ABC$. Sestrojíme obdélníky $ABKL$ a~$ACMN$ takové, že $|BK| = |AC|$ a~$|CM| = |AB|$, přičemž $K$ a~$L$ leží v~polorovině určené přímkou $AB$, která neobsahuje $C$, a~$M$ a~$N$ leží v~polorovině určené přímkou $AC$, která neobsahuje $B$. Dokažte, že přímky $BN$, $CL$ a~$KM$ procházejí jedním bodem.
}{
...
\Problem{0}{}{
    ...
    Podívejme se na $ABC$ a~$AXE$. Ty jsou přímo podobné, tedy spirálka dává, že $ABX$ a~$ACE$ jsou podobné. Nedává nám to nějaké dobré úhly?
}{
    ...
    Podobnost $ABC \sim AXE$ je spirální podobnost se středem $A$ zobrazující $BC$ na $XE$, a~jelikož spirálka chodí po dvou, $A$ je i~středem spirální podobnosti zobrazující $BX$ na $CE$. Trojúhelníky $ABX$ a~$ACE$ jsou tak přímo podobné, což spolu s~$ABC \sim CDE$ dává
    $$
...
\Problem{0}{}{
    ...
    Naše spirálka přenáší $BI_1$ na $EI_2$, tím pádem nějaká jiná spirálka se středem $D$ přenáší $BE$ na $I_1I_2$. Tahle spirálka nám už dává dobrou podobnost.
}{
...
}%
{
    ...
    Spirální podobnost chodí po dvou, takže $D$ je i~středem spirální podobnosti zobrazující $BE$ na $I_1I_2$, a~proto $|\angle DI_1I_2| = |\angle DBE| = 90^\circ - \gamma$. Bod $I_1$ je střed kružnice vepsané trojúhelníku $BFD$, takže $|\angle BI_1D| = 90^\circ + |\angle BFD|/2 = 90^\circ + \gamma/2$. Polopřímky $DI_1$ a~$DI_2$ leží v~úhlech $BDF$ a~$EDC$, takže $B$ a~$I_2$ jsou v~opačných polorovinách určených přímkou $DI_1$ a
    $$
    |\angle BI_1I_2| = |\angle BI_1D| + |\angle DI_1I_2| = 180^\circ - \gamma/2.
    $$
    Dále $CI_2$ je osa úhlu $DCE$ velikosti $\gamma$, takže $|\angle I_2CB| = \gamma/2$. Bod $I_2$ leží uvnitř úhlu $CBI_1$, takže $I_1$ a~$C$ jsou v~opačných polorovinách určených přímkou $BI_2$, a~ze součtu $|\angle BI_1I_2| + |\angle I_2CB| = 180^\circ$ leží $B$, $I_1$, $I_2$, $C$ na kružnici.
}
...
\Problem{0}{CPS 2014, 3, zobecněná}{
    ...
    Pořád nevíme, co s~pravými úhly, zkusme dokreslit něco, co pravé úhly vyrobí. V~dalším hintu to prozradím.
}{
    Trikem je označit paty z~$C$ na $BM$ a~$DN$, označme je $X$ a~$Y$. Díky pravým úhlům stačí nějaká tětivovost. Není jasné, jak jsme si pomohli, ale faktem je, že jsme ještě vůbec nepoužili úhlové podmínky ze zadání.
}{
    ...
    Znamená to, že $X$ dělí $BM$ ve stejném poměru jako $Y$ dělí $DN$. Takže naše krásná spirálka přenáší $X$ na $Y$. Už jsme skoro v~cíli. Spojme to, co přenáší, s~tím, jak se najde její střed.
}%
{
    Přímky $BM$ a~$DN$ se protínají v~$A$, takže podle věty o~středu spirální podobnosti je $K$ středem spirální podobnosti zobrazující $BD$ na $MN$. Spirální podobnost chodí \uv{po dvou}, takže $K$ je i~středem spirální podobnosti $\sigma$ zobrazující $BM$ na $DN$.
    ...
    Polopřímky $CM$ a~$CN$ jsou souměrně sdružené podle osy úhlu $BCD$, která zaměňuje polopřímky $CB$ a~$CD$, takže $M$ leží uvnitř úsečky $AB$ právě tehdy, když $N$ leží uvnitř úsečky $AD$; z~$|\angle ABC| = |\angle ADC|$ je proto $|\angle MBC| = |\angle NDC|$ a~$\triangle CMB \sim \triangle CND$ (nesouhlasně orientované, bereme z~ní jen poměr). Odpovídají si v~ní paty výšek z~$C$, tedy
    $$
    |BX| : |MX| = |DY| : |NY|,
    $$
    a~spirální podobnost přenáší \uv{podobné body}, takže $\sigma(X) = Y$.

    Tedy $\sigma$ zobrazuje $XB$ na $YD$ a~jelikož spirálka chodí po dvou, je $K$ i~středem spirální podobnosti zobrazující $XY$ na $BD$. Přímky $XB$ a~$YD$ se protínají v~$A$, takže podle věty o~středu je tento střed druhým průsečíkem kružnic $(AXY) = \omega$ a~$(ABD)$. Proto $K \in \omega$, čili $|\angle AKC| = 90^\circ$.
}

\EnvId{xt9itZAMHcJDcCVjylaYP-teziste-rovnostrannych-trojuhelniku}
\Problem{0}{Napoleonova věta}{
    Nechť $ABC$ je trojúhelník a~nechť body $D$, $E$, $F$ jsou takové, že trojúhelníky $BCD$, $CAE$ a~$ABF$ jsou rovnostranné, přičemž $D$ a~$A$ leží v~opačných polorovinách určených přímkou $BC$, body $E$ a~$B$ leží v~opačných polorovinách určených přímkou $CA$ a~body $F$ a~$C$ leží v~opačných polorovinách určených přímkou $AB$. Dokažte, že těžiště těchto trojúhelníků rovněž tvoří rovnostranný trojúhelník.
}{
...
}%
{
    ...
    Spirálka chodí po dvou, a~tak je $A$ i~středem spirálky zobrazující $XY$ na $CF$, tedy
    $$
    |XY| : |CF| = |AX| : |AC| = k.
    $$
    ...
    Otočení se středem $A$ o~$60^\circ$ zobrazující $F$ na $B$ zobrazuje i~$C$ na $E$, protože otočení zobrazující $C$ na $E$ má díky podmínce na poloroviny stejný směr; proto $|CF| = |BE|$. Stejně tak otočení se středem $C$ o~$60^\circ$ zobrazuje $A$ na $E$ a~$D$ na $B$, takže $|AD| = |BE|$. Proto $|XY| = |YZ| = |ZX|$ a~trojúhelník $XYZ$ je rovnostranný.
}
...
\Problem{0}{ISL 2002, G4}{
    ...
    Ukazuje se, že jeho úhly jsou pevné, z~věty o~obvodovém a~středovém úhlu je $|\angle A_1OQ| = 2|\angle A_1A_2Q| = 2|\angle PA_2Q|$. To nám může dobře naznačit, proč se $O$ pohybuje tak, jak tvrdí zadání.
}{
    Idea je uvědomit si, že $O$ je obraz vhodného bodu v~nějaké vhodné spirálce.
}%
{
    Přímky $A_1A_2$ a~$B_1B_2$ se protínají v~$P$ a~kružnice opsané trojúhelníkům $A_1B_1P$ a~$A_2B_2P$ jsou $\omega_1$ a~$\omega_2$, takže podle věty o~středu spirální podobnosti je jejich druhý průsečík $Q$ středem spirální podobnosti $\sigma$ zobrazující $A_1A_2$ na $B_1B_2$. Spirálka chodí \uv{po dvou}, a~tak je $Q$ i~středem spirální podobnosti zobrazující $A_1B_1$ na $A_2B_2$.
    ...
    $$
    také pevný. Rovnoramenný trojúhelník s~daným úhlem při hlavním vrcholu má daný tvar, takže poměr $|QO| : |QA_1|$ i~orientovaný úhel od polopřímky $QA_1$ k~polopřímce $QO$ vyjdou pro každé $A_1$ stejně.
...
\Problem{0}{IMO 2005, 5}{
    ...
    Jak nápadně naznačuje předchozí hint, chceme dokázat, že $S$ je ještě na jedné kružnici, té ze zadání. To už je jednoduché úhlení, jen v~tom bordelu nemusí být hned vidět.
}%
{
    ...
    Zobrazení $\sigma$ převádí $AF$ na $CE$, a~jelikož spirálka chodí po dvou, je $S$ i~středem spirální podobnosti zobrazující $AC$ na $FE$. Tyto přímky se protínají v~$R$, a~tak podle věty o~středu spirální podobnosti leží $S$ na kružnici $AFR$. Stejně tak $\sigma$ převádí $DF$ na $BE$, tedy $DB$ na $FE$, a~$S$ leží na kružnici $DFQ$.
...
\Problem{0}{MEMO 2010, T6}{
    ...
    Z~dokázaných vlastností máme, že $FS$ prochází jedním z~Miquelových bodů $ABCD$. Na jakých všemožných kružnicích ten bod leží?
}{
    ...
    Trik je dokázat, že trojúhelníky $GCA$ a~$DCE$ jsou podobné. Jeden úhel se dá najít snadno. Co dál?
}{
...
{
    Nechť $G$ je střed spirální podobnosti zobrazující $AB$ na $CD$. Podle věty o~dalších Miquelových bodech leží $G$ na kružnici $ACF$ a~jelikož $ABCD$ je tětivový, i~na přímce $SF$, a~to na téže straně od $F$ jako $S$.
...
\Problem{0}{USA TST 2007}{
    ...
    Všimněme si, že $T$ je střed kružnice opsané $B_1C_1CB$, takže $|\angle TAS| = 90^\circ$ je jedna z~našich vlastností, které jsme dokazovali. K~dalším by se hodily ještě nějaké kružnice. Nabízejí se $SC_1CA$ a~$SB_1BA$. Ty se ale nedají snadno vyúhlit (myslím). Naštěstí jich je fakt dost.
}{
    Co se vyúhlit dá, je to, že průsečík $X$ přímek $B_1B$ a~$C_1C$ leží na kružnici $ABC$. Nebude to už stačit?
}{
...
\Problem{0}{}{
    ...
    Když dokreslíme střed $D$ úsečky $EF$, tak spirálka převádějící $EDF$ na $BMC$ má střed v~$P$, takže $P$, $M$, $D$, $R$ jsou na kružnici, která díky pravým úhlům prochází i~bodem $O$. Věřte nebo ne, už se to dá elegantně vyúhlit.
}{
    Doúhleme to tak, že si uvědomíme, že stačí $|\angle EPO| = 90^\circ - |\angle ABC|$. Ten ale vyjádříme jako $|\angle EPR| - |\angle OPR|$.
}%
{
    ...
    Miquelův bod čtyřúhelníku $BEFC$ (s~$A = BE \cap CF$ a~$R = BC \cap EF$) leží na kružnicích $ABC$, $AEF$ a~$RBE$; jelikož $A$ na kružnici $RBE$ neleží, je jím $P$. Čtyřúhelník $RBEP$ je tedy tětivový a~$P$ je střed spirální podobnosti zobrazující $EF$ na $BC$.

    Nechť $D$ je střed úsečky $EF$. Podle věty o~přenášení podobných bodů jde $D$ na $M$, tedy $DE$ na $MB$. Přímky $DE$ a~$MB$ se protínají v~$R$ a~$M$ na $EF$ neleží, takže podle věty o~středu spirální podobnosti je $P$ druhý průsečík kružnic $DMR$ a~$EBR$.
...
\Problem{0}{ISL 1992}{
    Konvexní čtyřúhelník $ABCD$ má stejně dlouhé úhlopříčky. Nad každou jeho stranou sestrojíme rovnoramenný trojúhelník, jehož základnou je tato strana a~jehož hlavní vrchol leží v~té z~polorovin určených přímkou této strany, která neobsahuje zbylé dva vrcholy čtyřúhelníku. Všechny čtyři trojúhelníky jsou navzájem podobné. Dokažte, že spojnice hlavních vrcholů trojúhelníků nad protilehlými stranami jsou na sebe kolmé.
}{
    ...
    Poslední krok je uvědomit si, že najednou řešíme o~dost jinou úlohu: dokazujeme, že když máme čtyřúhelník se stejně dlouhými úhlopříčkami, tak spojnice středů protilehlých stran jsou na sebe kolmé. To už je poloznámé. V~dalších hintech to rozvedu víc.
}{
...
{
    Podle věty o~středu spirální podobnosti (s~$X = A$) je $A_2$ středem spirálky, která zobrazuje $C_1B_1$ na $BC$. Spirálka chodí po dvou, takže $A_2$ je i~středem spirálky zobrazující $C_1B$ na $B_1C$, tedy $A_2C_1B \sim A_2B_1C$ a
    $$
...
\Problem{0}{ISL 2000, G6}{
    ...
    Trikem je vzít na polopřímce $YM$ bod $T$ takový, že $ADX$ a~$AYT$ jsou přímo podobné. Jenže my vlastně nevíme, jestli $|\angle AYT| = |\angle ADX|$, takže nemáme jistotu, že existuje. Bod $Y$ ale umíme předefinovat, aby to bylo fajn.
}{
...
}%
{
    ...
    Nechť $T$ je obraz bodu $X$ ve spirální podobnosti se středem $A$, která zobrazuje $D$ na $Y'$. Potom platí $ADX \sim AY'T$ a~oba trojúhelníky jsou souhlasně orientované, takže $|\angle AY'T| = \delta = |\angle AY'M|$, a~jelikož $X$ leží uvnitř $ABCD$ a~$Y'$ v~polorovině $ABC$, což je pro konvexní $ABCD$ polorovina $ABD$, jsou trojice $A, D, X$ a~$A, Y', M$ obě orientované jako $A, D, B$ a~$T$ leží na polopřímce $Y'M$.

    \Image{spiral-interior-point-x.pdf}

    Osová souměrnost podle $Y'M$ zobrazuje $A$ na $B$ a~body $Y'$, $T$ nechává na místě, takže $BY'T$ je nesouhlasně shodný s~$AY'T$, a~proto platí $BCX \sim BY'T$ a~také tyto trojúhelníky jsou souhlasně orientované.

    Spirální podobnost se středem $A$ zobrazuje $DX$ na $Y'T$, a~jelikož spirální podobnost chodí po dvou, je $A$ i~středem té, která zobrazuje $DY'$ na $XT$. Odtud $AY'D \sim ATX$, čili
    $$
    |Y'D| : |TX| = |AD| : |AX|.
    $$
