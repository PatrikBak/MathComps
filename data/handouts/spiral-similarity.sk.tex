...
\Definition{Špirálna podobnosť}{
    \textit{Špirálna podobnosť} je geometrické zobrazenie, ktoré je zložením otočenia a~rovnoľahlosti. Je určená stredom, orientovaným uhlom otočenia~$\phi$ a~koeficientom rovnoľahlosti $k>0$.
...
\EnvId{JmA51D6zLoKYiJEsuQBWn-spiralna-podobnost-po-dvoch}
\Theorem{Špirálna podobnosť chodí \uv{po dvoch}}{
    ...
    Nech má špirálna podobnosť uhol $\phi$ a~koeficient $k$. Priamo z~definície je $\angle AOA' = \angle BOB' = \phi$ (orientované uhly) a~$|OA'| : |OA| = |OB'| : |OB| = k$.

    Pripočítaním uhla $A'OB$ k~obom stranám prvej rovnosti dostávame $\angle AOB = \angle A'OB'$ a~preusporiadaním druhej $|OB| : |OA| = |OB'| : |OA'|$. Špirálna podobnosť so stredom $O$, uhlom $\angle AOB$ a~koeficientom $|OB| : |OA|$ teda zobrazuje $A$ na $B$ a~$A'$ na $B'$, čiže $AA'$ na $BB'$.
}

\EnvId{7w5OPOXaDCNOVEgIpddYa-spiralna-podobnost-a-podobne-body}
\Theorem{Špirálna podobnosť prenáša \uv{podobné body}}{
    Nech $S$ je špirálna podobnosť so~stredom~$O$ prevádzajúca $AB$ na $A'B'$. Nech $X$ a~$X'$ sú body na úsečkách $AB$ a~$A'B'$ také, že $|AX|:|BX|=|A'X'|:|B'X'|$. Potom $S$ prenáša $X$ na $X'$, a~ak $S$ nie je rovnoľahlosť a~$O$ nie je žiadny z~bodov $A$, $B$, $X$, sú trojuholníky $OAA'$, $OXX'$, $OBB'$ navzájom podobné.

    \Image{spiral-similar-points.pdf}
}{
    Podobnosť $S$ zobrazuje úsečku $AB$ na úsečku $A'B'$ a~zachováva pomery dĺžok, takže $S(X)$ je bod úsečky $A'B'$, ktorý ju delí v~pomere $|AX| : |BX|$. Taký bod je jediný, preto $S(X) = X'$.

    Trojuholníky $OAA'$, $OXX'$, $OBB'$ majú pri vrchole $O$ ten istý uhol $\phi$ špirálnej podobnosti a~ten istý pomer strán $|OA'| : |OA| = |OX'| : |OX| = |OB'| : |OB| = k$, takže sú podobné podľa vety $sus$.
}

\EnvId{eAqIhVm0OVC2uVXDRxMpJ-stred-spiralnej-podobnosti}
\Theorem{Stred špirálnej podobnosti}{
    Nech $A,B,C,D$ sú body v~rovine také, že $AB \nparallel CD$. Nech sa priamky $AB$ a~$CD$ pretínajú v~$X$, ktorý je rôzny od $A$, $B$, $C$, $D$. Nech sa kružnice opísané trojuholníkom $XAC$ a~$XBD$ pretínajú znova v~$O$; ak sa v~$X$ dotýkajú, položme $O = X$. Potom $O$ je stred jednoznačnej špirálnej podobnosti, ktorá zobrazuje $AB$ na $CD$.

    \Image{spiral-center.pdf}
}{
    Počítame s~orientovanými uhlami priamok modulo $180^\circ$. Z~kružnice $XAOC$ je $\angle(AO, AB) = \angle(AO, AX) = \angle(CO, CX) = \angle(CO, CD)$ a~z~kružnice $XBOD$ rovnako $\angle(BO, BA) = \angle(DO, DC)$, takže trojuholníky $OAB$ a~$OCD$ sú súhlasne podobné. (Ak sa kružnice v~$X$ dotýkajú, prevedie ich jedna na druhú rovnoľahlosť so stredom $X$; tá zobrazuje $A$ na $B$ a~$C$ na $D$, lebo priamka $AB$ pretína prvú kružnicu v~$X$ a~$A$ a~druhú v~$X$ a~$B$, a~podobne pre $CD$. Preto $|XB| : |XA| = |XD| : |XC|$ a~špirálna podobnosť so stredom $X$ zobrazujúca $A$ na $C$ zobrazuje aj $B$ na $D$.)

    \Image{spiral-center-proof.pdf}

    Zo súhlasnej podobnosti je $|OA| : |OC| = |OB| : |OD|$ a~rovnajú sa aj orientované uhly polpriamok $\angle AOB = \angle COD$, čiže po pripočítaní uhla $BOC$ aj $\angle AOC = \angle BOD$. Podľa vety $sus$ sú preto súhlasne podobné aj trojuholníky $OAC$ a~$OBD$, a~to presne znamená, že špirálna podobnosť so stredom $O$, uhlom $\angle AOC$ a~koeficientom $|OC| : |OA|$ zobrazuje $A$ na $C$ a~$B$ na $D$. Iná špirálna podobnosť zobrazujúca $AB$ na $CD$ neexistuje: priama podobnosť je určená obrazmi dvoch bodov, lebo každý ďalší bod je určený vzdialenosťami od nich a~orientáciou.
}

Keďže špirálna podobnosť chodí po dvoch, ten istý bod $O$ je aj stredom špirálnej podobnosti, ktorá zobrazuje $AC$ na $BD$. Ak naviac $AC \nparallel BD$, dáva rovnaký postup pre túto dvojicu: priamky $AC$ a~$BD$ sa pretnú v~bode $Y$ a~$O$ je druhým priesečníkom kružníc opísaných trojuholníkom $ABY$ a~$CDY$.
...
\i Ak $AB \parallel CD$ a~$AC \nparallel BD$, prvá konštrukcia zlyhá (priamky $AB$ a~$CD$ sa nepretínajú), druhá však funguje: stredom je $Y$ a~špirálna podobnosť zobrazujúca $AB$ na $CD$ je rovnoľahlosťou so stredom $Y$, so záporným koeficientom, ak $Y$ leží medzi $A$ a~$C$, s~kladným, ak mimo úsečky $AC$. Kružnice $ABY$ a~$CDY$ sa vtedy v~$Y$ dotýkajú, zvonka, resp. zvnútra.
   \Image{spiral-center-tangent.pdf}
   \Image{spiral-center-tangent-internal.pdf}
\i Ak $AB \parallel CD$ a~zároveň $AC \parallel BD$, zlyhajú obe konštrukcie. Štvoruholník $ABDC$ je vtedy rovnobežník a~zobrazenie $A$ na $C$, $B$ na $D$ je posunutie, ktoré stred nemá.
\enditems
...
\Theorem{Miquelov bod}{
    ...
    Priamky $AB$ a~$DC$ sa pretínajú v~$Q$, takže podľa vety o~strede špirálnej podobnosti je stred $M$ špirálnej podobnosti zobrazujúcej $AB$ na $DC$ druhým priesečníkom kružníc $QAD$ a~$QBC$.

    Špirálka chodí po dvoch, takže $M$ je aj stredom špirálnej podobnosti zobrazujúcej $AD$ na $BC$. Priamky $AD$ a~$BC$ sa pretínajú v~$R$, a~tak tá istá veta dáva, že $M$ je druhým priesečníkom kružníc $RAB$ a~$RDC$. Všetky štyri kružnice teda prechádzajú bodom $M$.
}
...
\Theorem{Miquelov bod tetivového štvoruholníka}{
    ...
    Počítame s~orientovanými uhlami priamok modulo $180^\circ$. Z~kružníc $QADM$ a~$RABM$, ktoré dáva predošlá veta, je
    $$
    \gather
    ...
    Bod $M$ leží na $QR$ práve vtedy, keď priamky $MQ$ a~$MR$ splývajú, teda keď $\angle(MQ, MA) = \angle(MR, MA)$. To je podľa vypísaných rovností práve vtedy, keď $\angle(DC, DA) = \angle(BC, BA)$, a~to je presne podmienka tetivovosti $ABCD$.
}

Označme ešte $P = AC \cap BD$. Ten istý postup pre zvyšné dve dvojice priamok dáva dva ďalšie Miquelove body:
\begitems
\i Kružnice opísané trojuholníkom $PAB, PCD, QAC, QBD$ prechádzajú jedným bodom $M_1$, stredom špirálnej podobnosti zobrazujúcej $AB$ na $CD$, resp. $AC$ na $BD$. Tento bod leží na $PQ$ práve vtedy, keď $ABCD$ je tetivový.
   \Image{spiral-miquel-others-pq.pdf}
\i Kružnice opísané trojuholníkom $PAD, PCB, RAC, RDB$ prechádzajú jedným bodom $M_2$, stredom špirálnej podobnosti zobrazujúcej $AD$ na $CB$, resp. $AC$ na $DB$. Tento bod leží na $PR$ práve vtedy, keď $ABCD$ je tetivový.
...
\Theorem{$OM \perp QR$}{
    ...
    Označme $X$ a~$Y$ stredy úsečiek $AD$ a~$BC$. Bod $M$ je stredom špirálnej podobnosti zobrazujúcej $AD$ na $BC$ a~tá podľa vety o~prenášaní podobných bodov zobrazuje $X$ na $Y$, teda aj úsečku $XD$ na $YC$. Priamky $XD = AD$ a~$YC = BC$ sa pretínajú v~$R$, takže podľa vety o~strede špirálnej podobnosti je $M$ druhým priesečníkom kružníc $RXY$ a~$RDC$.

    \Image{spiral-miquel-om-perpendicular-proof.pdf}

    Stred $O$ kružnice opísanej leží na osiach tetív $AD$ a~$BC$, takže pri $X$ aj $Y$ je pravý uhol a~kružnica $RXY$ je kružnicou s~priemerom $OR$. Leží na nej aj $M$, a~preto $|\angle OMR| = 90^\circ$. Bod $M$ leží na $QR$ (predošlá veta), takže $OM \perp QR$.
}
...
\Theorem{$O$, $P$, $M$ na priamke}{
    ...
    Najprv ukážeme, že $A$, $O$, $M$, $C$ ležia na kružnici; počítame s~orientovanými uhlami priamok modulo $180^\circ$. Bod $M$ je stredom špirálnej podobnosti zobrazujúcej $A$ na $D$ a~$B$ na $C$, ktorej uhol je $\angle(AB, DC)$, a~zároveň špirálnej podobnosti zobrazujúcej $A$ na $B$ a~$D$ na $C$ s~uhlom $\angle(AD, BC)$. Preto je $\angle(MA, MD) = \angle(AB, DC)$ a~zároveň $\angle(MD, MC) = \angle(AD, BC)$. Z~tetivovosti $ABCD$ je navyše $\angle(BC, DC) = \angle(AB, AD)$, a~tak
    $$
    \gather
    ...
    \endgather
    $$
    Podľa vety o~obvodovom a~stredovom uhle je $2\angle(AB, BC) = \angle(OA, OC)$, takže $A$, $O$, $M$, $C$ ležia na kružnici. Rovnako ležia na kružnici aj $B$, $O$, $M$, $D$. (Ak je $AC$ alebo $BD$ priemer, dáva tá istá rovnosť $M$ na tomto priemere a~$O$, $P$, $M$ ležia na ňom priamo.)

    \Image{spiral-miquel-opm-line-proof.pdf}

    Obe kružnice prechádzajú bodmi $O$ a~$M$, ich chordálou je teda priamka $OM$. Bod $P$ leží vnútri tetív $AC$ a~$BD$, teda vnútri oboch kružníc, a~jeho mocnosť vzhľadom na ne je $-|PA| \cdot |PC|$, resp. $-|PB| \cdot |PD|$; obe sa rovnajú mocnosti $P$ vzhľadom na kružnicu opísanú $ABCD$. Bod $P$ teda leží na chordále $OM$ a~kolmosť $OP \perp QR$ je predošlá veta.
}
...
Ukážeme ešte jednu dôležitú vlastnosť z~projektívnej geometrie súvisiacu s~polárami.

\EnvId{1LKVhRCTK_6pAfJ02sHWt-inverzia-a-polara}
\Definition{Polára}{
    Nech $\omega$ je kružnica so stredom $O$ a~polomerom $r$ a~nech $P \neq O$ je bod. Obraz bodu $P$ v~\textit{inverzii} vzhľadom na $\omega$ je bod $P'$ polpriamky $OP$, pre ktorý $|OP| \cdot |OP'| = r^2$. \textit{Polára} bodu $P$ vzhľadom na $\omega$ je priamka prechádzajúca bodom $P'$ kolmá na $OP$.
}

\EnvId{8WFRq4Y9hX4Xf2REA_qfL-polara}
\Theorem{Polára}{
    Nech $ABCD$ je tetivový štvoruholník. Označme $P=AC \cap BD$, $R=AD \cap BC$ a~$Q=AB \cap CD$. Potom $QR$ je polára bodu $P$ vzhľadom na kružnicu opísanú $ABCD$ so stredom $O$, analogicky $PR$ je polára ku $Q$ a~$PQ$ je polára ku $R$.

    \Image{spiral-miquel-polar.pdf}
}{
    Označme $r$ polomer kružnice opísanej a~$M$ Miquelov bod na $QR$. Podľa predošlej vety ležia $O$, $P$, $M$ na priamke a~$A$, $O$, $M$, $C$ na kružnici (ak je $AC$ priemer, vezmeme kružnicu $BOMD$ a~všade ďalej píšeme $BD$ namiesto $AC$); bod $P$ leží vnútri tetivy $AC$, teda vnútri tejto kružnice, a~tak aj vnútri jej tetivy $OM$. Mocnosť bodu $P$ vzhľadom na ňu dáva $|PO| \cdot |PM| = |PA| \cdot |PC| = r^2 - |OP|^2$, čiže
    $$
    |OP| \cdot |OM| = |OP| \cdot \bigl(|OP| + |PM|\bigr) = r^2.
    $$
    Bod $M$ je teda obrazom bodu $P$ v~inverzii vzhľadom na kružnicu opísanú.

    \Image{spiral-miquel-polar-proof.pdf}

    Polára bodu $P$ je preto priamka prechádzajúca bodom $M$ kolmá na $OM$. Bod $M$ leží na $QR$ a~$OM \perp QR$, takže touto priamkou je $QR$. Pre body $Q$ a~$R$ je úvaha analogická, len tie ležia mimo kružnice opísanej, takže ich mocnosť je kladná a~obraz v~inverzii padne medzi stred a~daný bod.
}

\sec Ďalšie známe konfigurácie

\EnvId{tQ6r-YBHII2hYOcd0jnzC-ptolemaiova-veta}
\Theorem{Ptolemaiova veta}{
    Nech $ABCD$ je konvexný štvoruholník so stranami $a=|AB|$, $b=|BC|$, $c=|CD|$, $d=|DA|$ a~uhlopriečkami $e=|AC|$, $f=|BD|$. Potom
    $$
    ac + bd \geq ef,
    $$
    ...
    Nech $X$ je obraz bodu $D$ v~špirálnej podobnosti so stredom $A$, ktorá zobrazuje $C$ na $B$. Trojuholníky $ACD$ a~$ABX$ sú potom súhlasne podobné s~koeficientom $a/e$, takže $|BX| = ac/e$. Špirálka chodí po dvoch, a~tak je $A$ aj stredom špirálnej podobnosti zobrazujúcej $CB$ na $DX$: trojuholníky $ACB$ a~$ADX$ sú súhlasne podobné s~koeficientom $d/e$ a~$|DX| = bd/e$.

    \Image{spiral-ptolemy-proof.pdf}

    Z~trojuholníkovej nerovnosti je
    $$
    f = |BD| \leq |BX| + |XD| = {ac + bd \over e},
    $$
    pričom rovnosť nastáva práve vtedy, keď $X$ leží na úsečke $BD$. Z~podobností je $|\angle ABX| = |\angle ACD|$ a~$|\angle ADX| = |\angle ACB|$, a~pre konvexný $ABCD$ sú trojuholníky $ABX$ a~$ABD$ súhlasne orientované a~rovnako $ADX$ a~$ADB$. Bod $X$ tak leží na polpriamke $BD$ práve vtedy, keď $|\angle ABD| = |\angle ACD|$, a~na polpriamke $DB$ práve vtedy, keď $|\angle ADB| = |\angle ACB|$. Úsečka $BD$ je prienikom týchto dvoch polpriamok a~každá z~rovností je ekvivalentná s~tetivovosťou $ABCD$, takže rovnosť nastáva práve pre tetivový $ABCD$.
}

\EnvId{ECXlxtdJwECBnuRZW8E-V-simsonova-priamka}
\Theorem{Simsonova priamka}{
    ...
    Označme $X$, $Y$, $Z$ päty kolmíc z~bodu $D$ na priamky $AB$, $AC$, $BC$ a~počítajme s~orientovanými uhlami priamok modulo $180^\circ$. Z~tetivovosti $ABCD$ je $\angle(BD, BX) = \angle(BD, BA) = \angle(CD, CA) = \angle(CD, CY)$ a~pri $X$ aj $Y$ je pravý uhol, takže pravouhlé trojuholníky $DXB$ a~$DYC$ sú súhlasne podobné. Špirálna podobnosť $\sigma$ so stredom $D$ zobrazujúca $B$ na $X$ preto zobrazuje $C$ na $Y$. (Pre $X = B$ je $AD$ priemer, vtedy aj $Y = C$ a~všetky tri päty ležia na $BC$.)

    \Image{spiral-simson-proof.pdf}

    Nech $P$ je vzor bodu $Z$ v~$\sigma$. Trojuholník $DZP$ je súhlasne podobný s~$DXB$, takže $|\angle DZP| = 90^\circ$ a~$P$ leží na kolmici na $DZ$ v~bode $Z$, čo je priamka $BC$. Body $B$, $P$, $C$ teda ležia na priamke, a~tak na priamke ležia aj ich obrazy $X$, $Z$, $Y$.
}

\EnvId{J67DMaac8DHSjoZgc7hzh-sharky-devil}
\Theorem{Sharky-Devil}{
    Nech sa kružnica vpísaná trojuholníku $ABC$ dotýka strán $BC$, $CA$, $AB$ po rade v~bodoch $D$, $E$, $F$ a~nech sa kružnice opísané trojuholníkom $AEF$ a~$ABC$ pretínajú znova v~bode $S$ (pre $|AB| = |AC|$ sa dotýkajú v~$A$ a~vtedy je $S = A$). Potom priamka $SD$ prechádza stredom oblúka $BC$ neobsahujúceho $A$, čiže $SD$ je osou uhla $BSC$.

    \Image{spiral-sharky-devil.pdf}
}{
    Priamky $FB$ a~$EC$ sa pretínajú v~$A$ a~kružnice $AFE$ a~$ABC$ sa pretínajú znova v~$S$, takže podľa vety o~strede špirálnej podobnosti je $S$ stredom špirálnej podobnosti zobrazujúcej $FB$ na $EC$. Jej uhol je orientovaný uhol od polpriamky $FB$ k~polpriamke $EC$, čiže orientovaný uhol $BAC$, a~rovná sa orientovanému uhlu $BSC$; bod $S$ teda vidí $BC$ pod rovnakým orientovaným uhlom ako $A$ a~leží na oblúku $BC$ obsahujúcom $A$.

    Trojuholníky $SFB$ a~$SEC$ sú podobné a~dotyčnice z~jedného bodu sú rovnako dlhé, $|BF| = |BD|$ a~$|CE| = |CD|$, takže
    $$
    {|SB| \over |SC|} = {|FB| \over |EC|} = {|DB| \over |DC|}.
    $$
    Podľa obrátenej vety o~osi uhla je $SD$ osou uhla $BSC$ a~os obvodového uhla $BSC$ prechádza stredom $N$ oblúka $BC$ neobsahujúceho $S$, teda neobsahujúceho $A$.
}
...
\Theorem{}{
    ...
    Označme $N$ druhý priesečník kružníc $ADE$ a~$ABC$ (pre $|AB| = |AC|$ sa dotýkajú v~$A$, ktoré je vtedy samo stredom oblúka $BAC$). Priamky $DB$ a~$EC$ sa pretínajú v~$A$, takže podľa vety o~strede špirálnej podobnosti je $N$ stredom špirálnej podobnosti zobrazujúcej $DB$ na $EC$. Jej koeficient je $|EC| : |DB| = 1$, ide teda o~otočenie a~trojuholníky $NDB$ a~$NEC$ sú zhodné. Špeciálne $|NB| = |NC|$, čiže $N$ je stredom jedného z~oblúkov $BC$.

    \Image{spiral-arc-midpoint-proof.pdf}

    Uhol otočenia je orientovaný uhol od polpriamky $DB$ k~polpriamke $EC$, čiže orientovaný uhol $BAC$, a~rovná sa orientovanému uhlu $BNC$. Bod $N$ teda vidí $BC$ pod rovnakým orientovaným uhlom ako $A$ a~leží na oblúku $BC$ obsahujúcom $A$.
}
...
}%
{
    ...
    Špirálka chodí po dvoch, takže $A$ je aj stredom špirálnej podobnosti $T$, ktorá zobrazuje $BB'$ na $CC'$ a~$B \mapsto C$. Jej uhol otočenia je preto $|\angle BAC| = 90^\circ$, teda $CC' = T(BB') \perp BB'$.
}
...
}%
{
    ...
    Trojuholníky $TBK$ a~$TCL$ sú pravouhlé rovnoramenné s~pravým uhlom pri $K$, resp. $L$, a~keďže $K$ a~$L$ ležia v~polrovinách $BTC$ a~$CTA$ a~$T$ je vnútorný bod trojuholníka $ABC$, sú orientované súhlasne. Existuje teda špirálna podobnosť so stredom $T$ prevádzajúca $BK$ na $CL$. Špirálka chodí po dvoch, takže $T$ je aj stredom špirálnej podobnosti $\sigma$, ktorá prevádza $BC$ na $KL$.

    \Image{spiral-centroid-isosceles.pdf}

    Podobnosť zachováva stredy úsečiek, preto $\sigma(D) = E$, a~keďže špirálka chodí po dvoch, je $T$ aj stredom špirálnej podobnosti prevádzajúcej $BK$ na $DE$. Trojuholníky $TBK$ a~$TDE$ sú tak podobné, čiže $TDE$ je pravouhlý rovnoramenný s~preponou $TD$ a~$|TD| = \sqrt2 \, |DE|$. Ťažisko delí ťažnicu $AD$ v~pomere $|AT| : |TD| = 2 : 1$, a~tak
    $$
    |AT| : |DE| = 2\sqrt2.
    $$
}
...
{
    Priamky $XY$ a~$BC$ sa pretínajú v~$D$, takže podľa vety o~strede špirálnej podobnosti je druhý priesečník kružníc $(XBD) = (ABD)$ a~$(YCD) = (ACD)$, teda bod $A$, stredom špirálnej podobnosti zobrazujúcej $XB$ na $YC$. Špirálka chodí po dvoch, takže $A$ je aj stredom špirálnej podobnosti $S$ zobrazujúcej $XY$ na $BC$.

    Body $M'$ a~$M$ sú stredy úsečiek $XY$ a~$BC$, a~keďže špirálka prenáša podobné body, $S(M') = M$. Podobnosť $S$ teda zobrazuje $XM'$ na $BM$, a~keďže špirálka chodí po dvoch, je $A$ stredom špirálnej podobnosti zobrazujúcej $XB$ na $M'M$.
...
\Problem{0}{}{
    Je daný ostrouhlý trojuholník $ABC$. Zostrojíme obdĺžniky $ABKL$ a~$ACMN$ také, že $|BK| = |AC|$ a~$|CM| = |AB|$, pričom $K$ a~$L$ ležia v~polrovine určenej priamkou $AB$, ktorá neobsahuje $C$, a~$M$ a~$N$ ležia v~polrovine určenej priamkou $AC$, ktorá neobsahuje $B$. Dokážte, že priamky $BN$, $CL$ a~$KM$ prechádzajú jedným bodom.
}{
...
{
    Obdĺžniky $ABKL$ a~$ANMC$ sú zhodné v~tomto poradí vrcholov (lebo $|AN| = |AB|$ a~$|NM| = |BK|$), majú spoločný vrchol $A$ a~sú súhlasne orientované, lebo oba ležia mimo trojuholníka. Otočenie so stredom $A$ tak zobrazuje $BK$ na $NM$. Špirálka chodí po dvoch, takže $A$ je aj stredom špirálnej podobnosti zobrazujúcej $BN$ na $KM$; jej uhol $|\angle BAK|$ nie je ani nulový, ani priamy, a~preto $BN \nparallel KM$. Ich priesečník označme $X$. Otočenie, ktoré zobrazuje $B$ na $N$, má uhol $90^\circ + |\angle BAC| < 180^\circ$, takže $A$ neleží na priamke $BN$ a~$X \neq A$. Podľa vety o~strede špirálnej podobnosti je $A$ druhým priesečníkom kružníc $XBK$ a~$XNM$.
...
\Problem{0}{}{
    ...
    Podobnosť $ABC \sim AXE$ je špirálna podobnosť so stredom $A$ zobrazujúca $BC$ na $XE$, a~keďže špirálka chodí po dvoch, $A$ je aj stredom špirálnej podobnosti zobrazujúcej $BX$ na $CE$. Trojuholníky $ABX$ a~$ACE$ sú tak priamo podobné, čo spolu s~$ABC \sim CDE$ dáva
    $$
...
}%
{
    ...
    Špirálna podobnosť chodí po dvoch, takže $D$ je aj stredom špirálnej podobnosti zobrazujúcej $BE$ na $I_1I_2$, a~preto $|\angle DI_1I_2| = |\angle DBE| = 90^\circ - \gamma$. Bod $I_1$ je stred kružnice vpísanej trojuholníku $BFD$, takže $|\angle BI_1D| = 90^\circ + |\angle BFD|/2 = 90^\circ + \gamma/2$. Polpriamky $DI_1$ a~$DI_2$ ležia v~uhloch $BDF$ a~$EDC$, takže $B$ a~$I_2$ sú v~opačných polrovinách určených priamkou $DI_1$ a
    $$
    |\angle BI_1I_2| = |\angle BI_1D| + |\angle DI_1I_2| = 180^\circ - \gamma/2.
    $$
    Ďalej $CI_2$ je os uhla $DCE$ veľkosti $\gamma$, takže $|\angle I_2CB| = \gamma/2$. Bod $I_2$ leží vo vnútri uhla $CBI_1$, takže $I_1$ a~$C$ sú v~opačných polrovinách určených priamkou $BI_2$, a~zo súčtu $|\angle BI_1I_2| + |\angle I_2CB| = 180^\circ$ ležia $B$, $I_1$, $I_2$, $C$ na kružnici.
}
...
{
    Priamky $BM$ a~$DN$ sa pretínajú v~$A$, takže podľa vety o~strede špirálnej podobnosti je $K$ stredom špirálnej podobnosti zobrazujúcej $BD$ na $MN$. Špirálna podobnosť chodí \uv{po dvoch}, takže $K$ je aj stredom špirálnej podobnosti $\sigma$ zobrazujúcej $BM$ na $DN$.
    ...
    Teda $\sigma$ zobrazuje $XB$ na $YD$ a~keďže špirálka chodí po dvoch, je $K$ aj stredom špirálnej podobnosti zobrazujúcej $XY$ na $BD$. Priamky $XB$ a~$YD$ sa pretínajú v~$A$, takže podľa vety o~strede je tento stred druhým priesečníkom kružníc $(AXY) = \omega$ a~$(ABD)$. Preto $K \in \omega$, čiže $|\angle AKC| = 90^\circ$.
}

\EnvId{xt9itZAMHcJDcCVjylaYP-taziska-rovnostrannych-trojuholnikov}
\Problem{0}{Napoleonova veta}{
    Nech $ABC$ je trojuholník a~nech body $D$, $E$, $F$ sú také, že trojuholníky $BCD$, $CAE$ a~$ABF$ sú rovnostranné, pričom $D$ a~$A$ ležia v~opačných polrovinách určených priamkou $BC$, body $E$ a~$B$ ležia v~opačných polrovinách určených priamkou $CA$ a~body $F$ a~$C$ ležia v~opačných polrovinách určených priamkou $AB$. Dokážte, že ťažiská týchto trojuholníkov taktiež tvoria rovnostranný trojuholník.
}{
...
}%
{
    ...
    Špirálka chodí po dvoch, a~tak je $A$ aj stredom špirálky zobrazujúcej $XY$ na $CF$, teda
    $$
    |XY| : |CF| = |AX| : |AC| = k.
    $$
    ...
    Otočenie so stredom $A$ o~$60^\circ$ zobrazujúce $F$ na $B$ zobrazuje aj $C$ na $E$, lebo otočenie zobrazujúce $C$ na $E$ má vďaka podmienke na polroviny rovnaký smer; preto $|CF| = |BE|$. Rovnako otočenie so stredom $C$ o~$60^\circ$ zobrazuje $A$ na $E$ a~$D$ na $B$, takže $|AD| = |BE|$. Preto $|XY| = |YZ| = |ZX|$ a~trojuholník $XYZ$ je rovnostranný.
}
...
}%
{
    ...
    Zobrazenie $\sigma$ prevádza $AF$ na $CE$, a~keďže špirálka chodí po dvoch, je $S$ aj stredom špirálnej podobnosti zobrazujúcej $AC$ na $FE$. Tieto priamky sa pretínajú v~$R$, a~tak podľa vety o~strede špirálnej podobnosti leží $S$ na kružnici $AFR$. Rovnako $\sigma$ prevádza $DF$ na $BE$, teda $DB$ na $FE$, a~$S$ leží na kružnici $DFQ$.
...
{
    Nech $G$ je stred špirálnej podobnosti zobrazujúcej $AB$ na $CD$. Podľa vety o~ďalších Miquelových bodoch leží $G$ na kružnici $ACF$ a~keďže $ABCD$ je tetivový, aj na priamke $SF$, a~to na tej istej strane od $F$ ako $S$.
...
\Problem{0}{}{
    ...
    Douhlime tak, že si uvedomíme, že stačí $|\angle EPO| = 90^\circ - |\angle ABC|$. Ten ale vyjadríme ako $|\angle EPR| - |\angle OPR|$.
}%
{
    ...
    Miquelov bod štvoruholníka $BEFC$ (s~$A = BE \cap CF$ a~$R = BC \cap EF$) leží na kružniciach $ABC$, $AEF$ a~$RBE$; keďže $A$ na kružnici $RBE$ neleží, je ním $P$. Štvoruholník $RBEP$ je teda tetivový a~$P$ je stred špirálnej podobnosti zobrazujúcej $EF$ na $BC$.

    Nech $D$ je stred úsečky $EF$. Podľa vety o~prenášaní podobných bodov ide $D$ na $M$, teda $DE$ na $MB$. Priamky $DE$ a~$MB$ sa pretínajú v~$R$ a~$M$ na $EF$ neleží, takže podľa vety o~strede špirálnej podobnosti je $P$ druhý priesečník kružníc $DMR$ a~$EBR$.
...
\Problem{0}{ISL 1992}{
    Konvexný štvoruholník $ABCD$ má rovnako dlhé uhlopriečky. Nad každou jeho stranou zostrojíme rovnoramenný trojuholník, ktorého základňou je táto strana a~ktorého hlavný vrchol leží v~polrovine určenej priamkou tejto strany, ktorá neobsahuje zvyšné dva vrcholy štvoruholníka. Všetky štyri trojuholníky sú navzájom podobné. Dokážte, že spojnice hlavných vrcholov trojuholníkov nad protiľahlými stranami sú na seba kolmé.
}{
...
{
    Podľa vety o~strede špirálnej podobnosti (s~$X = A$) je $A_2$ stredom špirálky, ktorá zobrazuje $C_1B_1$ na $BC$. Špirálka chodí po dvoch, takže $A_2$ je aj stredom špirálky zobrazujúcej $C_1B$ na $B_1C$, teda $A_2C_1B \sim A_2B_1C$ a
    $$
...
}%
{
    ...
    Špirálna podobnosť so stredom $A$ zobrazuje $DX$ na $Y'T$, a~keďže špirálna podobnosť chodí po dvoch, je $A$ aj stredom tej, ktorá zobrazuje $DY'$ na $XT$. Odtiaľ $AY'D \sim ATX$, čiže
    $$
    |Y'D| : |TX| = |AD| : |AX|.
    $$
