...
\Problem{0}{}{
    ...
    $$
    f(x)\bigl(2f(0)-1\bigr) = 1. \eqno(1)
    $$
    Were $2f(0)-1$ zero, (1) would read $0=1$. Hence $2f(0)-1\neq0$ and
    $$
    f(x) = \frac{1}{2f(0)-1},
    $$
...
\Problem{0}{}{
    ...
    $$
    so $f$ is injective. Putting $x=0$ in the given equation gives $f(f(0))=f(0)$, and injectivity turns that into $f(0)=0$. In particular $f(f(0))=0$. Any further solution of $f(f(x))=0$ would satisfy $f(f(x))=f(f(0))=f(0)$, and hence $x=0$. The only solution is therefore $x=0$.
}
...
\Problem{0}{}{
    ...
    f(x)^2 = x^2 \eqno(1)
    $$
    for every real $x$. That is our familiar trap; on its own it yields neither $f(x)\equiv x$ nor $f(x)\equiv-x$. But (1) holds for every number, $x-y$ included, so the left-hand side of the given equation is $(x-y)^2$; canceling the equal terms and dividing by $-2$ leaves
    $$
    f(x)f(y) = xy. \eqno(2)
    $$
    Now $f$ is not the zero function, or else the left-hand side of (2) would always be zero, while for $x=y=1$ the right-hand side is not. So pick $b$ with $f(b)\neq0$ and set $c=b/f(b)$; the choice $y=b$ in (2) gives $f(x)f(b)=xb$, so $f(x)=cx$. Back in (2), $c^2xy=xy$, so $c^2=1$ and $c=\pm1$.
...
\Problem{0}{}{
    ...
    $$
    f(x)-f(x+y) = yf(x)f(x+y). \eqno(1)
    $$
    The symmetric expression here is $f(x+y)$, since swapping $x$ and $y$ leaves the argument $x+y$ alone. So let us solve (1) for it; as $1+yf(x)$ is positive,
    $$
    f(x+y) = \frac{f(x)}{1+yf(x)}. \eqno(2)
    $$
    Swapping $x$ and $y$ leaves the left-hand side of (2) alone, so it must leave the right-hand side alone too. Cross-multiplying gives $f(x)+xf(x)f(y)=f(y)+yf(x)f(y)$, and dividing by $f(x)f(y)>0$,
    $$
    \frac{1}{f(x)}-x = \frac{1}{f(y)}-y. \eqno(3)
    $$
    Fix $y$ in (3). The right-hand side is then a constant; call it $c$. So $f(x)=1/(x+c)$ for every $x>0$. From $c=1/f(x)-x$ and $f(x)>0$ we get $c>-x$ for every $x>0$, and if $c$ were negative, the choice $x=-c/2$ would give $c>c/2$, a contradiction. Hence $c\ge0$.
...
\Problem{0}{Czech-Slovak 2017}{
    ...
    $$
    f(xy) = f(x)y+x^2f(y). \eqno(2)
    $$
    The left-hand side of (2) is symmetric in $x$ and $y$, so we may swap the variables and compare the two right-hand sides; rearranging,
    $$
    f(x)\,y(1-y) = f(y)\,x(1-x). \eqno(3)
    $$
    The choices $y=0$ and $y=1$ in (3) merely zero out both sides, so take $y=2$ instead. It gives $-2f(x)=f(2)\,x(1-x)$, and with $c=f(2)/2$,
    $$
    f(x) = c(x^2-x).
    $$
...
\Problem{0}{Czech-Slovak 2002}{
    ...
    $$
    f\bigl(f(x)f(y)\bigr) = f\bigl(f(x)y\bigr)+f(x). \eqno(1)
    $$
    The term $f(f(x)y)$ is in the way, so let us solve for it using the given equation with the roles of the variables swapped, $f\bigl(yf(x)\bigr)=f(xy)+y$. Then (1) becomes
    $$
    f\bigl(f(x)f(y)\bigr) = f(xy)+y+f(x). \eqno(2)
    $$
    Swapping $x$ and $y$ leaves both the left-hand side of (2) and the term $f(xy)$ alone, so comparing (2) with its own swap gives $y+f(x)=x+f(y)$. So $f(x)-x$ is constant, $f(x)=x+c$. In the given equation the left-hand side is now $f\bigl(x(y+c)\bigr)=xy+xc+c$ and the right-hand side $xy+c+x$, so $xc=x$ and $c=1$.
...
\Problem{0}{Czech-Slovak 2011}{
    ...
    $$
    f(xf(y)) = f(x)-\frac{1}{xyf(y)}. \eqno(1)
    $$
    The argument $xf(y)$ becomes symmetric in $x$ and $y$ if we put $f(x)$ in place of $x$. Indeed, $x:=f(x)$ in (1) gives $f\bigl(f(x)f(y)\bigr)=f\bigl(f(x)\bigr)-\frac{1}{yf(x)f(y)}$, whose left-hand side survives a swap of $x$ and $y$ untouched. Hence
    $$
    f\bigl(f(x)\bigr)-\frac{1}{yf(x)f(y)} = f\bigl(f(y)\bigr)-\frac{1}{xf(x)f(y)}. \eqno(2)
    $$
    Let $a=f(1)$. Putting $y=1$ in (2) yields one expression for $f(f(x))$,
    $$
    f\bigl(f(x)\bigr) = f(a)+\frac{x-1}{axf(x)}, \eqno(3)
    $$
    while the original equation with $x=1$, $y:=x$ reads $af(x)=f(x)f\bigl(f(x)\bigr)+\frac1x$ and yields a second,
    $$
    f\bigl(f(x)\bigr) = a-\frac{1}{xf(x)}. \eqno(4)
    $$
    In particular $x=1$ in (4) gives $f(a)=a-\frac1a$. Substituting that into (3), comparing with (4) and multiplying by $axf(x)$,
    $$
    \eqalign{
...
\Problem{0}{}{
    ...
    After substituting $x=0$ we have $f(f(y))=y+f^2(0)$, from which we can prove the bijection. This relation is nice; it would be useful to compute $f(0)$.
}{
    The key to computing $f(0)$ is a trick: From the shape of the equation I suspect that the solution will be $f(x)=x$, so $f(0)$ will probably be $0$. From surjectivity we know that the function is $0$ somewhere; say $f(a)=0$. Let us try to substitute suitably.
}{
    The key is $x=a$ into the original equation; that gives us directly $f(f(y))=y$, which compared with $f(f(y))=y+f^2(0)$ gives $f(0)=0$.
}{
    ...
    That is our familiar trap; (4) does not yet make $f$ identically $x$ or identically $-x$. So suppose there are nonzero $a$, $b$ with $f(a)=a$ and $f(b)=-b$. The choice $x=a$, $y=b$ in (1) gives $f(a^2-b)=b+a^2$, while by (4) $f(a^2-b)$ equals $a^2-b$ or $-(a^2-b)$. The first case forces $b=0$, the second $a=0$, either way a contradiction. Since the two formulas agree at zero, either $f(x)=x$ for all $x$, or $f(x)=-x$ for all $x$.
...
\Problem{0}{IMO 1992 P2}{
    ...
    Why does $f$ have a zero? Well, for $y=-f^2(x)$ we have $f(\hbox{something})=0$. But how exactly do we compute it? It takes a few substitutions. Let $f(a)=0$...
}{
    ...
    To prove that it is increasing, let us first substitute $y:=f(y)$, which gives us $f(x^2+y)=f(y)+f^2(x)$. This already implies that it is increasing, even though we cannot see it yet.
}{
    ...
    First notice that $f$ has a zero: the choice $y=-f^2(x)$ makes the right-hand side zero, so $f$ vanishes at the point $x^2+f(y)$. So let $f(a)=0$ and let us try to compute $a$. Substituting $x=y=a$ gives
    $$
    f(a^2) = a, \eqno(1)
    $$
    and the choice $x=0$, $y=a^2$ gives $f\bigl(f(a^2)\bigr)=a^2+f^2(0)$, which by (1) reads $0=a^2+f^2(0)$. Both terms are nonnegative, so $a=0$ and also $f(0)=0$.

    Now $x=0$ gives us
    $$
    f\bigl(f(y)\bigr) = y \eqno(2)
    $$
    for all real $y$, so $f$ is injective, and together with $f(0)=0$ it vanishes only at zero.

    Plain substitution takes us no further here, and an idea is needed: we show that $f$ is increasing. Relation (2) lets us put $y:=f(y)$ in the given equation, which unwinds the inner double $f$ and leaves
    $$
    f(x^2+y) = f(y)+f^2(x). \eqno(3)
    $$
    Let $u>v$. The choice $y=v$ and $x=\sqrt{u-v}>0$ makes $x^2+y=u$, so (3) gives $f(u)=f(v)+f^2\bigl(\sqrt{u-v}\bigr)$. The added square is positive, since $f$ vanishes only at zero. Therefore $f(u)>f(v)$.

    But an increasing function satisfying (2) can only be the identity: if $f(x)>x$, monotonicity gives $f\bigl(f(x)\bigr)>f(x)>x$, and if $f(x)<x$ it gives $x=f\bigl(f(x)\bigr)<f(x)<x$, each contradicting (2).
...
\Problem{0}{MEMO 2015 T2}{
    ...
    $$
    f(p^3y) = f(y)+p^3-b. \eqno(3)
    $$
    Apply relation (3) three times in a row, successively for $y=1$, $y=p^3$ and $y=p^6$:
    $$
...
\Problem{0}{}{
    ...
    Let us first try to guess the solution. The right-hand side looks a lot like the expansion of $(x+f(y))^2$, and indeed $f(x)=x^2$ satisfies the equation, as does $f(x)=x^2+c$ for an arbitrary constant $c$. That leads us to track how far $f$ is from the square, so write $g(x)=f(x)-x^2$. Substituting $f(x)=g(x)+x^2$, the left-hand side of the given equation becomes $g\bigl(x+f(y)\bigr)+x^2+2xf(y)+f^2(y)$ and the right-hand side $g(x)+x^2+f^2(y)+2xf(y)$. Almost everything cancels, leaving $g(x+f(y))=g(x)$, and since $f(y)=y^2+g(y)$,
    $$
    ...
    $$
    g(x) = g\bigl(x+p(2y+p)\bigr) \eqno(2)
    $$
    ...
    Check: for the zero function both sides are zero; for $f(x)=x^2+c$ the left-hand side is $\bigl(x+f(y)\bigr)^2+c=x^2+2xf(y)+f^2(y)+c$ and the right-hand side $x^2+c+f^2(y)+2xf(y)$.
}
...
\Problem{0}{MEMO 2012 I1}{
    ...
    $$
    f\Bigl(x+\frac1y\Bigr) = \frac{1}{x+\frac1y}. \eqno(4)
    $$
    Now let $t\le1$ be an arbitrary positive number and choose $y>1/t$; then $y>1$ and also $1/y<t$, so $x=t-1/y>0$ makes $x+\frac1y=t$ and (4) gives $f(t)=1/t$. Together with (3) we therefore have $f(x)=1/x$ for all $x>0$.
