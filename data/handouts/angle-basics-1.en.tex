...
\Exercise{}{
    ...
    Let $\alpha$ denote the angle that the transversal $AD$ makes with the parallel lines on the side of $B$ (as in the figure). The corresponding angles $\alpha$ at~$A$ and $\alpha$ at~$D$ are equal. At vertex $A$ the angles $\alpha$ and $\beta$ form a linear pair, so
    $$
    \beta + \alpha = 180^\circ.
    $$
...
\Example{}{
    ...
    \Remark Note that along the way we essentially proved the triangle inequality: the sum of two sides of a triangle is greater than the third -- our triangle is constructible and non-degenerate~\fnote{has no angle equal to $180^\circ$} if and only if $b+c>a$.
}
...
A very handy fact useful in non-trivial problems is the following. One possible proof goes via trigonometry.
