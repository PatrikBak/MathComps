...
\Subtitle{Proof Basics}

\MathcompsLink{proofs-intro}
...
\Problem{0}{}{
    Prove that the sum of three odd squares is never a square.
}{
    The key is to look at divisibility by eight.
}{
    Use the previously proven example.
}{
    According to the previous example, the square of an odd number leaves a remainder of~1 when divided by~8. The sum of three such numbers therefore leaves a remainder of~3. But then it cannot be a square, since that would have to be odd and, again according to our statement, leave a remainder of~1.
}

In harder problems, it may not be immediately obvious how to use the assumptions from the problem statement.
...
    Each divisor is in the form $p_1^{b_1} \cdot p_2^{b_2} \cdots p_k^{b_k}$, where $0 \leq b_i \leq a_i$ for each $1 \leq i \leq k$. For the choice of $b_i$ we have $a_i+1$ options ($0,1,\ldots,a_i$). These choices are independent, so according to the combinatorial rule of product, we have a total of
    $$
    (a_1+1)(a_2+1)\cdots (a_k+1)
    $$
    distinct $k$-tuples $(b_1,\ldots,b_k)$ corresponding to all divisors of $n$, so we are done.
}

\Exercise{}{
    Solve the previous example using the theorem on the number of divisors, including the reverse implication.
}{
