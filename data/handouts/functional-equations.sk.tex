...
\Problem{0}{}{
    ...
    $$
    Keby $2f(0)-1=0$, dávala by (1) rovnosť $0=1$. Preto $2f(0)-1\neq0$ a
    $$
    f(x) = \frac{1}{2f(0)-1},
    $$
...
\Problem{0}{}{
    ...
    $$
    a~$f$ je prostá. Ak do zadania dosadíme $x=0$, získame $f(f(0))=f(0)$, a~z~prostoty platí $f(0)=0$. Tým sme dostali aj platnosť $f(f(0))=0$. Ak teraz existuje ešte nejaké riešenie $f(f(x))=0$, tak $f(f(x))=f(f(0))=f(0)$, a~teda $x=0$. Jediným riešením je preto $x=0$.
}
...
\Problem{0}{}{
    ...
    f(x)^2 = x^2 \eqno(1)
    $$
    pre každé reálne $x$. To je naša známa pasca, sama osebe ešte nedáva $f(x)\equiv x$ ani $f(x)\equiv-x$. Vzťah (1) však platí pre ľubovoľné číslo, teda aj pre $x-y$, takže ľavá strana zadania je $(x-y)^2$ a~po odčítaní rovnakých členov a~vydelení číslom $-2$ zostane
    $$
    f(x)f(y) = xy. \eqno(2)
    $$
    Funkcia $f$ nie je nulová, lebo inak by ľavá strana (2) bola stále nulová, kým pravá pre $x=y=1$ nie je. Nech teda $f(b)\neq0$ a~$c=b/f(b)$; voľba $y=b$ v~(2) dáva $f(x)f(b)=xb$, čiže $f(x)=cx$. Späť v~(2) to dáva $c^2xy=xy$, odkiaľ $c^2=1$ a~$c=\pm1$.
...
\Problem{0}{}{
    ...
    $$
    Symetrickým výrazom je tu $f(x+y)$, lebo pri výmene $x$ a~$y$ sa argument $x+y$ nezmení. Vyjadrime si ho preto z~(1); vďaka kladnosti $1+yf(x)$ máme
    $$
    f(x+y) = \frac{f(x)}{1+yf(x)}. \eqno(2)
    $$
    Ľavá strana (2) sa výmenou $x$ a~$y$ nezmení, a~teda sa nesmie zmeniť ani pravá. Po vynásobení krížom je $f(x)+xf(x)f(y)=f(y)+yf(x)f(y)$ a~po vydelení kladným $f(x)f(y)$
    $$
    \frac{1}{f(x)}-x = \frac{1}{f(y)}-y. \eqno(3)
    $$
    Zafixujme v~(3) $y$; pravá strana je konštanta, označme ju $c$. Potom $f(x)=1/(x+c)$ pre každé $x>0$. Z~$c=1/f(x)-x$ a~$f(x)>0$ máme $c>-x$ pre každé $x>0$, a~keby bolo $c<0$, voľba $x=-c/2$ by dala $c>c/2$, spor. Preto $c\ge0$.
...
\Problem{0}{Czech-Slovak 2017}{
    ...
    $$
    Ľavá strana (2) je symetrická v~$x$ a~$y$, preto smieme premenné vymeniť a~obe pravé strany porovnať; po preusporiadaní
    $$
    f(x)\,y(1-y) = f(y)\,x(1-x). \eqno(3)
    $$
    Voľby $y=0$ a~$y=1$ v~(3) iba vynulujú obe strany, vezmime teda napríklad $y=2$. Dostaneme $-2f(x)=f(2)\,x(1-x)$, čiže po označení $c=f(2)/2$
    $$
    f(x) = c(x^2-x).
    $$
...
\Problem{0}{Czech-Slovak 2002}{
    ...
    $$
    Prekážajúce $f(f(x)y)$ vyjadríme zo zadania s~vymenenými úlohami premenných, $f\bigl(yf(x)\bigr)=f(xy)+y$, čím (1) prejde na
    $$
    f\bigl(f(x)f(y)\bigr) = f(xy)+y+f(x). \eqno(2)
    $$
    Ľavá strana (2) aj výraz $f(xy)$ sa zámenou $x$ a~$y$ nezmenia, takže porovnaním (2) s~rovnosťou po takejto zámene máme $y+f(x)=x+f(y)$. Výraz $f(x)-x$ je teda konštantný, $f(x)=x+c$. Dosadením do zadania je ľavá strana $f\bigl(x(y+c)\bigr)=xy+xc+c$ a~pravá $xy+c+x$, odkiaľ $xc=x$ a~$c=1$.
...
\Problem{0}{Czech-Slovak 2011}{
    ...
    $$
    Argument $xf(y)$ sa stane symetrickým v~$x$ a~$y$, ak za $x$ dosadíme $f(x)$. Voľba $x:=f(x)$ v~(1) dáva $f\bigl(f(x)f(y)\bigr)=f\bigl(f(x)\bigr)-\frac{1}{yf(x)f(y)}$, kde sa ľavá strana zámenou $x$ a~$y$ nezmení, takže
    $$
    f\bigl(f(x)\bigr)-\frac{1}{yf(x)f(y)} = f\bigl(f(y)\bigr)-\frac{1}{xf(x)f(y)}. \eqno(2)
    $$
    Označme $a=f(1)$. Voľba $y=1$ v~(2) dáva prvé vyjadrenie
    $$
    ...
    $$
    Špeciálne $x=1$ v~(4) dáva $f(a)=a-\frac1a$. Po dosadení do (3), porovnaní s~(4) a~vynásobení výrazom $axf(x)$ máme
    $$
    \eqalign{
...
\Problem{0}{}{
    ...
    Po dosadení $x=0$ máme $f(f(y))=y+f^2(0)$, z~čoho dokážeme bijekciu. Tento vzťah je fajn, hodilo by sa spočítať $f(0)$.
}{
    Kľúčom k~spočítaniu $f(0)$ je trik: Podľa tvaru rovnice tuším, že riešením bude $f(x)=x$, teda $f(0)$ bude asi $0$. Zo surjektivity vieme, že funkcia je niekde $0$, nech $f(a)=0$. Skúsme vhodne dosadiť.
}{
    Kľúčom je $x=a$ do pôvodnej rovnice, to nám dáva rovno $f(f(y))=y$, čo v~porovnaní s~$f(f(y))=y+f^2(0)$ dáva $f(0)=0$.
}{
    ...
    To je naša známa pasca, zo (4) ešte nevyplýva, že $f$ je identicky $x$ alebo identicky $-x$. Predpokladajme teda, že existujú nenulové $a$, $b$ také, že $f(a)=a$ a~$f(b)=-b$. Voľba $x=a$, $y=b$ v~(1) dáva $f(a^2-b)=b+a^2$, kým podľa (4) je $f(a^2-b)$ rovné $a^2-b$ alebo $-(a^2-b)$. V~prvom prípade $b=0$, v~druhom $a=0$, oboje spor. Keďže v~nule oba predpisy splývajú, platí buď $f(x)=x$ pre všetky $x$, alebo $f(x)=-x$ pre všetky $x$.
...
\Problem{0}{IMO 1992 P2}{
    ...
    Prečo má $f$ nulový bod? Nuž, pre $y=-f^2(x)$ máme $f(\hbox{niečo})=0$. Ako ale presne spočítame? Chce to pár dosadení. Nech $f(a)=0$...
}{
    ...
    Na dôkaz rastúcosti najprv dosaďme $y:=f(y)$, čo nám dá $f(x^2+y)=f(y)+f^2(x)$. Rastúcosť z~toho už plynie, aj keď to nie je vidieť.
}{
    ...
    Najprv si všimnime, že $f$ má nulový bod: pri voľbe $y=-f^2(x)$ je pravá strana nulová, takže $f$ je nulová v~bode $x^2+f(y)$. Nech teda $f(a)=0$ a~pokúsme sa $a$ spočítať. Dosadenie $x=y=a$ dáva
    $$
    f(a^2) = a, \eqno(1)
    $$
    a~voľba $x=0$, $y=a^2$ dáva $f\bigl(f(a^2)\bigr)=a^2+f^2(0)$, čo je podľa (1) rovnosť $0=a^2+f^2(0)$. Oba sčítance sú nezáporné, a~preto $a=0$ aj $f(0)=0$.
    ...
    Tu už obyčajné dosadzovanie končí a~chce to myšlienku: ukážeme, že $f$ je rastúca. Vzťah (2) nám dovoľuje dosadiť do zadania $y:=f(y)$, čím sa vnútorné dvojité $f$ zjednoduší a~zostane
    $$
    f(x^2+y) = f(y)+f^2(x). \eqno(3)
    $$
    Nech $u>v$. Pri voľbe $y=v$ a~$x=\sqrt{u-v}>0$ je $x^2+y=u$, takže (3) dáva $f(u)=f(v)+f^2\bigl(\sqrt{u-v}\bigr)$, kde pripočítaný štvorec je kladný, lebo $f$ je nulová jedine v~nule. Preto $f(u)>f(v)$.

    Rastúca funkcia spĺňajúca (2) však už musí byť identita: z~$f(x)>x$ by rastúcosť dala $f\bigl(f(x)\bigr)>f(x)>x$ a~z~$f(x)<x$ zas $x=f\bigl(f(x)\bigr)<f(x)<x$, oboje spor s~(2).
...
\Problem{0}{MEMO 2015 T2}{
    ...
    $$
    Vzťah (3) použijeme trikrát za sebou, postupne pre $y=1$, $y=p^3$ a~$y=p^6$:
    $$
...
\Problem{0}{}{
    ...
    Skúška: pre nulovú funkciu sú obe strany nulové, pre $f(x)=x^2+c$ je ľavá strana $\bigl(x+f(y)\bigr)^2+c=x^2+2xf(y)+f^2(y)+c$ a~pravá $x^2+c+f^2(y)+2xf(y)$.
}
...
\Problem{0}{MEMO 2012 I1}{
    ...
    $$
    Nech je teraz $t\le1$ ľubovoľné kladné číslo a~zvoľme $y>1/t$; potom $y>1$ aj $1/y<t$, takže pre $x=t-1/y>0$ je $x+\frac1y=t$ a~(4) dáva $f(t)=1/t$. Spolu s~(3) je teda $f(x)=1/x$ pre všetky $x>0$.
