...
\Subtitle{Základy dôkazov}

\MathcompsLink{uvod-do-dokazov}
...
\Problem{0}{}{
    Dokážte, že súčet troch nepárnych druhých mocnín nikdy nie je druhou mocninou.
}{
    Kľúčom je pozrieť sa na deliteľnosť ôsmimi. 
}{
    Využite predošlý dokázaný príklad.
}{
   Podľa predošlého príkladu dáva druhá mocnina nepárneho čísla po delení~8 zvyšok~1. Súčet troch takýchto čísel preto dáva zvyšok~3. Potom ale nemôže byť štvorec, keďže to by musel byť nepárny a~znova podľa nášho tvrdenia dávať zvyšok~1.
}

V~ťažších úlohách nemusí byť hneď zrejmé, ako predpoklady zo zadania využiť. 
...
\Example{}{
    ...
    \textit{Poznámka.} Tento príklad v~skutočnosti platí ako ekvivalencia. V~skutočnosti by bol ľahší, ak by bola zadaná obrátená implikácia, lebo fakt, že je štvorec, sa v~úvahovom reťazci používa až na konci. V~súťažných úlohách je ale bežné, že autori sa snažia tieto kľúčové kroky zamaskovať.
}
...
\Theorem{}{
    ...
    Každý deliteľ je v~tvare $p_1^{b_1} \cdot p_2^{b_2} \cdots p_k^{b_k}$, kde $0 \leq b_i \leq a_i$ pre každé $1 \leq i \leq k$. Pre voľbu $b_i$ máme $a_i+1$ možností ($0,1,\ldots,a_i$). Tieto voľby sú nezávislé, takže podľa kombinatorického pravidla súčinu máme celkovo 
    $$
    (a_1+1)(a_2+1)\cdots (a_k+1)
    $$
    rôznych $k$-tíc $(b_1,\ldots,b_k)$ odpovedajúcich všetkým deliteľom $n$, takže sme hotoví.
}

\Exercise{}{
    Vyriešte predošlý príklad podľa vety o~počte deliteľov, vrátane obrátenej implikácie.
}{
...
\Problem{0}{}{
    ...
    \textit{Poznámka.} Typicky chybný dôkaz môže spočívať v~argumente, že súčet dvoch iracionálnych čísel je vždy iracionálny. Presvedčte sa ale, že to nie je pravda.
}
...
\Problem{2}{}{
    ...
    Predpokladajme, že existuje konečne veľa prvočísel tvaru $3n+2$.
    Označme $p_1, \dots, p_k$ všetky \textit{nepárne} prvočísla tohto tvaru (číslo 2 vynechávame). Uvažujme číslo
    $$
    N = 3p_1 \cdots p_k + 2.
    $$
