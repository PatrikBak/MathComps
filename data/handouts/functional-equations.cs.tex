...
\Problem{0}{}{
    ...
    $$
    Kdyby $2f(0)-1=0$, dávala by (1) rovnost $0=1$. Proto $2f(0)-1\neq0$ a
    $$
    f(x) = \frac{1}{2f(0)-1},
    $$
...
\Problem{0}{}{
    ...
    $$
    a~$f$ je prostá. Dosazením $x=0$ do zadání získáme $f(f(0))=f(0)$, a~z~prostoty tedy $f(0)=0$; tím máme i~$f(f(0))=0$. Je-li teď $x$ libovolné řešení rovnice $f(f(x))=0$, platí $f(f(x))=f(f(0))=f(0)$, a~tedy $x=0$. Jediným řešením je proto $x=0$.
}
...
\Problem{0}{}{
    ...
    f(x)^2 = x^2 \eqno(1)
    $$
    pro každé reálné $x$. To je naše známá past, sama o~sobě ještě nedává $f(x)\equiv x$ ani $f(x)\equiv-x$. Vztah (1) však platí pro libovolné číslo, tedy i~pro $x-y$, takže levá strana zadání je $(x-y)^2$ a~po odečtení stejných členů a~vydělení číslem $-2$ zůstane
    $$
    f(x)f(y) = xy. \eqno(2)
    $$
    Funkce $f$ není nulová, jinak by levá strana (2) byla vždy nulová, zatímco pravá pro $x=y=1$ nulová není. Nechť tedy $f(b)\neq0$ a~$c=b/f(b)$; volba $y=b$ v~(2) dává $f(x)f(b)=xb$, čili $f(x)=cx$. Dosazením zpět do (2) dostaneme $c^2xy=xy$, odkud $c^2=1$ a~$c=\pm1$.
...
\Problem{0}{}{
    ...
    $$
    Symetrickým výrazem je tu $f(x+y)$, protože při záměně $x$ a~$y$ se argument $x+y$ nezmění. Vyjádřeme si ho proto z~(1); díky kladnosti $1+yf(x)$ máme
    $$
    f(x+y) = \frac{f(x)}{1+yf(x)}. \eqno(2)
    $$
    Levá strana (2) se záměnou $x$ a~$y$ nezmění, a~tedy se nesmí změnit ani pravá. Po vynásobení křížem je $f(x)+xf(x)f(y)=f(y)+yf(x)f(y)$ a~po vydělení kladným $f(x)f(y)$
    $$
    \frac{1}{f(x)}-x = \frac{1}{f(y)}-y. \eqno(3)
    $$
    Zafixujme v~(3) $y$; pravá strana je konstanta, označme ji $c$. Potom $f(x)=1/(x+c)$ pro každé $x>0$. Z~$c=1/f(x)-x$ a~$f(x)>0$ máme $c>-x$ pro každé $x>0$, a~kdyby bylo $c<0$, volba $x=-c/2$ by dala $c>c/2$, spor. Proto $c\ge0$.
...
\Problem{0}{Czech-Slovak 2017}{
    ...
    $$
    Levá strana (2) je symetrická v~$x$ a~$y$, proto smíme proměnné vyměnit a~obě pravé strany porovnat; po úpravě
    $$
    f(x)\,y(1-y) = f(y)\,x(1-x). \eqno(3)
    $$
    Volby $y=0$ a~$y=1$ v~(3) jenom vynulují obě strany, vezměme tedy například $y=2$. Dostaneme $-2f(x)=f(2)\,x(1-x)$, čili po označení $c=f(2)/2$
    $$
    f(x) = c(x^2-x).
    $$
...
\Problem{0}{Czech-Slovak 2002}{
    ...
    $$
    Překážející $f(f(x)y)$ vyjádříme ze zadání, ve kterém vyměníme role proměnných: $f\bigl(yf(x)\bigr)=f(xy)+y$. Tím (1) přejde na
    $$
    f\bigl(f(x)f(y)\bigr) = f(xy)+y+f(x). \eqno(2)
    $$
    Levá strana (2) i~výraz $f(xy)$ se záměnou $x$ a~$y$ nezmění, takže porovnáním (2) s~rovností po takové záměně máme $y+f(x)=x+f(y)$. Výraz $f(x)-x$ je tedy konstantní, $f(x)=x+c$. Po dosazení do zadání je levá strana $f\bigl(x(y+c)\bigr)=xy+xc+c$ a~pravá $xy+c+x$, odkud $xc=x$ a~$c=1$.
...
\Problem{0}{Czech-Slovak 2011}{
    ...
    $$
    Argument $xf(y)$ se stane symetrickým v~$x$ a~$y$, pokud za $x$ dosadíme $f(x)$. Volba $x:=f(x)$ v~(1) dává $f\bigl(f(x)f(y)\bigr)=f\bigl(f(x)\bigr)-\frac{1}{yf(x)f(y)}$, kde se levá strana záměnou $x$ a~$y$ nezmění, takže
    $$
    f\bigl(f(x)\bigr)-\frac{1}{yf(x)f(y)} = f\bigl(f(y)\bigr)-\frac{1}{xf(x)f(y)}. \eqno(2)
    $$
    Označme $a=f(1)$. Volba $y=1$ v~(2) dává první vyjádření
    $$
    ...
    $$
    Speciálně $x=1$ v~(4) dává $f(a)=a-\frac1a$. Po dosazení do (3), porovnání s~(4) a~vynásobení výrazem $axf(x)$ máme
    $$
    \eqalign{
...
\Problem{0}{}{
    ...
    Po dosazení $x=0$ máme $f(f(y))=y+f^2(0)$, z~čehož dokážeme bijekci. Tento vztah je fajn, hodilo by se spočítat $f(0)$.
}{
    Klíčem ke spočítání $f(0)$ je trik: Podle tvaru rovnice tuším, že řešením bude $f(x)=x$, tedy $f(0)$ bude asi $0$. Ze surjektivity víme, že funkce je někde $0$, nechť $f(a)=0$. Zkusme vhodně dosadit.
}{
    Klíčem je $x=a$ do původní rovnice, to nám dává rovnou $f(f(y))=y$, což v~porovnání s~$f(f(y))=y+f^2(0)$ dává $f(0)=0$.
}{
    ...
    To je naše známá past, ze (4) ještě nevyplývá, že $f$ je identicky $x$ nebo identicky $-x$. Předpokládejme tedy, že existují nenulová $a$, $b$ taková, že $f(a)=a$ a~$f(b)=-b$. Volba $x=a$, $y=b$ v~(1) dává $f(a^2-b)=b+a^2$, zatímco podle (4) je $f(a^2-b)$ rovno $a^2-b$ nebo $-(a^2-b)$. V~prvním případě $b=0$, v~druhém $a=0$, obojí spor. Protože v~nule oba předpisy splývají, platí buď $f(x)=x$ pro všechna $x$, nebo $f(x)=-x$ pro všechna $x$.
...
\Problem{0}{IMO 1992 P2}{
    ...
    Proč má $f$ nulový bod? Inu, pro $y=-f^2(x)$ máme $f(\hbox{něco})=0$. Jak ale přesně spočítáme? Chce to pár dosazení. Nechť $f(a)=0$...
}{
    ...
    Na důkaz rostoucnosti nejprve dosaďme $y:=f(y)$, což nám dá $f(x^2+y)=f(y)+f^2(x)$. Rostoucnost z~toho už plyne, i~když to není vidět.
}{
    ...
    Nejprve si všimněme, že $f$ má nulový bod: při volbě $y=-f^2(x)$ je pravá strana nulová, takže $f$ je nulová v~bodě $x^2+f(y)$. Nechť tedy $f(a)=0$ a~pokusme se $a$ spočítat. Dosazení $x=y=a$ dává
    $$
    f(a^2) = a, \eqno(1)
    $$
    a~volba $x=0$, $y=a^2$ dává $f\bigl(f(a^2)\bigr)=a^2+f^2(0)$, což je podle (1) rovnost $0=a^2+f^2(0)$. Oba sčítance jsou nezáporné, a~proto $a=0$ i~$f(0)=0$.
    ...
    Tady už obyčejné dosazování končí a~chce to myšlenku: ukážeme, že $f$ je rostoucí. Vztah (2) nám dovoluje dosadit do zadání $y:=f(y)$, čímž se vnitřní dvojité $f$ zjednoduší a~zůstane
    $$
    f(x^2+y) = f(y)+f^2(x). \eqno(3)
    $$
    Nechť $u>v$. Při volbě $y=v$ a~$x=\sqrt{u-v}>0$ je $x^2+y=u$, takže (3) dává $f(u)=f(v)+f^2\bigl(\sqrt{u-v}\bigr)$, kde připočtený čtverec je kladný, protože $f$ je nulová jedině v~nule. Proto $f(u)>f(v)$.

    Rostoucí funkce splňující (2) však už musí být identita: z~$f(x)>x$ by rostoucnost dala $f\bigl(f(x)\bigr)>f(x)>x$ a~z~$f(x)<x$ zase $x=f\bigl(f(x)\bigr)<f(x)<x$, obojí spor s~(2).
...
\Problem{0}{MEMO 2015 T2}{
    ...
    $$
    Vztah (3) použijeme třikrát za sebou, postupně pro $y=1$, $y=p^3$ a~$y=p^6$:
    $$
...
\Problem{0}{}{
    ...
    Zkusme si řešení nejprve uhodnout. Pravá strana nápadně připomíná rozvinutý čtverec $(x+f(y))^2$, a~opravdu $f(x)=x^2$ rovnici splňuje, stejně jako $f(x)=x^2+c$ pro libovolnou konstantu $c$. To nás vede k~tomu, abychom sledovali, jak moc se $f$ od čtverce liší, tedy k~označení $g(x)=f(x)-x^2$. Po dosazení $f(x)=g(x)+x^2$ je levá strana zadání rovna $g\bigl(x+f(y)\bigr)+x^2+2xf(y)+f^2(y)$ a~pravá $g(x)+x^2+f^2(y)+2xf(y)$. Skoro všechno se vyruší a~zůstane $g(x+f(y))=g(x)$, čili díky $f(y)=y^2+g(y)$
    $$
    ...
    Zkouška: pro nulovou funkci jsou obě strany nulové, pro $f(x)=x^2+c$ je levá strana $\bigl(x+f(y)\bigr)^2+c=x^2+2xf(y)+f^2(y)+c$ a~pravá $x^2+c+f^2(y)+2xf(y)$.
}
...
\Problem{0}{MEMO 2012 I1}{
    ...
    $$
    Nechť je nyní $t\le1$ libovolné kladné číslo a~zvolme $y>1/t$; pak $y>1$ i~$1/y<t$, takže pro $x=t-1/y>0$ je $x+\frac1y=t$ a~(4) dává $f(t)=1/t$. Spolu s~(3) je tedy $f(x)=1/x$ pro všechna $x>0$.
