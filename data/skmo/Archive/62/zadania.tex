...
{%%%%%   CPSJ, priklad t3
V istej skupine je $n \ge 5$ osôb, pričom každé dve osoby, ktoré sa nepoznajú, majú práve jedného spoločného známeho a žiadna osoba nepozná všetky ostatné. Dokážte, že možno 5 spomedzi daných $n$ osôb usadiť za okrúhly stôl tak, aby každá z~nich sedela medzi svojimi
\ite a) známymi,
\ite b) neznámymi.
}
\podpis{...}
...
$$
Kružnica opísaná trojuholníku $ABC$ pretína strany $AD$ a $CD$ postupne v bodoch $K$ a $L$, rôznych od vrcholov štvor\-u\-hol\-níka. Úsečky $AL$ a $CK$ sa pretínajú v bode $P$. Dokážte, že $\angle ADB = \angle PDC$.
}
\podpis{...}

{%%%%%   CPSJ, priklad t5
Nech $a$, $b$, $c$ sú kladné reálne čísla, pre ktoré platí $ab+ac+bc\ge a+b+c$. Dokážte, že
$$
a+b+c\ge3.
$$}
\podpis{...}
...
{%%%%%   CPSJ, priklad t6
V~rovine je daný štvorec $ABCD$, kde $|AB|=a$. Určte najmenšiu možnú hodnotu polomerov
troch zhodných kruhov, pomocou ktorých je možné pokryť daný štvorec.}
\podpis{...}