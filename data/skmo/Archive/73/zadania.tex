...
{%%%%%   CPSJ, priklad t1
Označme $G$ ťažisko trojuholníka $ABC$. Nech $D$ je štvrtý vrchol rovnobežníka $AGDB$. Dokážte, že $BG \parallel CD$.}
\podpis{Patrik Bak}

{%%%%%   CPSJ, priklad t2
Medzi trojicami $(a,b,c)$ prirodzených čísel spĺňajúcich
$$
\left(a+14\sqrt{3}\right)\left(b-14c\sqrt{3}\right)=2024
$$
určete tú s najväčšou hodnotou $a$.}
\podpis{Mária Dományová}

{%%%%%   CPSJ, priklad t3
Určte veľkosti vnútorných uhlov vo všetkých rovnoramenných trojuholníkoch, ktoré možno rozdeliť na dva rovnoramenné trojuholníky s~disjunktnými vnútrami. }
\podpis{Jaroslav Švrček}

{%%%%%   CPSJ, priklad t4
Koľko je kladných celých čísel $n$ menších ako $2024$ a deliteľných $\lfloor \sqrt{n}\rfloor-1$? Symbol $\lfloor x\rfloor$ označuje najväčšie celé číslo nie väčšie ako $x$.

\nopagebreak\smallskip\noindent
Napríklad $n=8$ spĺňa danú podmienku, keďže $8$ je deliteľné $\lfloor\sqrt{8}\rfloor-1=2-1=1$, ale $n=9$ ju nespĺňa, keďže $9$ nie je deliteľné $\lfloor \sqrt{9}\rfloor -1=2$.}
\podpis{Patrik Bak}
...
{%%%%%  IMO, priklad 1
Určte všetky reálne čísla $\alpha$ také, že pre každé kladné celé číslo $n$ je celé číslo

$$
\lfloor\alpha\rfloor+\lfloor 2 \alpha\rfloor+\cdots+\lfloor n \alpha\rfloor
$$

násobkom $n$. (Zápisom $\lfloor z\rfloor$ označujeme najväčšie celé číslo neprevyšujúce $z$. Napríklad, $\lfloor-\pi\rfloor=-4$ a $\lfloor 2\rfloor=\lfloor 2.9\rfloor=2$.)}
\podpis{...}

{%%%%%  IMO, priklad 2
Určte všetky dvojice kladných celých čísel $(a, b)$, pre ktoré existujú kladné celé čísla $g$ a $N$ také, že

$$
\operatorname{NSD}\left(a^{n}+b, b^{n}+a\right)=g
$$

platí pre všetky celé čísla $n \geqslant N$. (Zápisom $\operatorname{NSD}(x, y)$ označujeme najväčšieho spoločného delitela celých čísel $x$ a $y$.)}
\podpis{...}

{%%%%%  IMO, priklad 3
Nech $a_{1}, a_{2}, a_{3}, \ldots$ je nekonečná postupnost kladných celých čísel a nech $N$ je kladné celé číslo. Predpokladajme, že pre každé $n>N$ je $a_{n}$ rovné počtu výskytov čísla $a_{n-1}$ medzi číslami $a_{1}, a_{2}, \ldots, a_{n-1}$.

Dokážte, že aspoň jedna z postupností $a_{1}, a_{3}, a_{5}, \ldots$ a $a_{2}, a_{4}, a_{6}, \ldots$ je časom periodická. (Nekonečná postupnosť $b_{1}, b_{2}, b_{3}, \ldots$ je časom periodická, ak existujú kladné celé čísla $p$ a $M$ také, že $b_{m+p}=b_{m}$ pre všetky $m \geqslant M$.)}
\podpis{...}

{%%%%%  IMO, priklad 4
Nech $A B C$ je trojuholník, v ktorom platí $|A B|<|A C|<|B C|$. Označme $\omega$ kružnicu vpísanú do trojuholníka $A B C$ a $I$ jej stred. Nech $X$ je bod na priamke $B C$ rôzny od bodu $C$ taký, že priamka prechádzajúca bodom $X$ rovnobežná s $A C$ sa dotýka $\omega$. Podobne, nech $Y$ je bod na priamke $B C$ rôzny od bodu $B$ taký, že priamka prechádzajúca bodom $Y$ rovnobežná s $A B$ sa dotýka $\omega$. Priamka $A I$ pretína kružnicu opísanú trojuholníku $A B C$ druhýkrát v bode $P \neq A$. Označme $K$ a $L$ postupne stredy strán $A C$ a $A B$.
Dokážte, že $|\angle K I L|+|\angle Y P X|=180^{\circ}$.}
\podpis{...}

{%%%%%  IMO, priklad 5
Slimák Turbo hrá hru v tabuľke s 2024 riadkami a 2023 stípcami. V 2022 políčkach tabuľky sa schovávajú príšery. Na začiatku Turbo nevie, kde sa príšery nachádzajú, ale vie, že v každom riadku, okrem prvého a posledného sa nachádza práve jedna príšera a v každom stípci sa nachádza maximálne jedna príšera.

Turbo robí sériu pokusov, aby sa dostal z prvého riadku do posledného. V každom pokuse začína na lubovoľnom políčku prvého riadku. Následne sa opakovane posúva do susedného políčka, ktoré zdieľa spoločnú stranu s jeho aktuálnym políčkom. (Turbo sa môže vrátit do už navštívených políčok.) Ak sa dostane do políčka s príšerou, jeho pokus končí a je premiestnený naspät do prvého riadku, aby začal nový pokus. Príšery sa nehýbu a Turbo si pamätá, či políčko, ktoré už navštívil, obsahuje príšeru, alebo nie. Ak sa dostane do lubovolného políčka v poslednom riadku, jeho pokus končí a tiež aj celá hra.

Určte najmenšiu hodnotu $n$, pre ktorú má Turbo stratégiu, ktorá zaručí dosiahnutie posledného riadku najneskôr v $n$-tom pokuse, bez ohľadu na rozmiestnenie príšer.}
\podpis{...}

{%%%%%  IMO, priklad 6
Nech $\mathbb{Q}$ je množina všetkých racionálnych čísel. Funkcia $f: \mathbb{Q} \rightarrow \mathbb{Q}$ sa nazýva kúpelná, ak má nasledujúcu vlastnost: Pre všetky $x, y \in \mathbb{Q}$ platí aspoň jedna z rovností

$$
f(x+f(y))=f(x)+y \quad \text { alebo } \quad f(f(x)+y)=x+f(y)
$$

Dokážte, že existuje celé číslo $c$ také, že pre lubovolnú kúpeľnú funkciu $f$ existuje najviac $c$ rôznych racionálnych čísel vyjadritelných v tvare $f(r)+f(-r)$ pre nejaké racionálne číslo $r$ a nájdite najmenšiu možnú hodnotu $c$.}
\podpis{...}