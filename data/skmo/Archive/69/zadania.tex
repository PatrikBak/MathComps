...
{%%%%%   vyberko, den 1, priklad 1
 Alexandra a~Benjamín stavajú stenu na ploche $1 \times m$ políčok. Striedavo umiestňujú tehly, pričom Alexandra umiestňuje zelené tehly a~Benjamín červené. Začína Alexandra. Výška steny nikdy nesmie presiahnuť~$n$ a~v~momente, keď je umiestnených $m \times n$ tehiel, hra končí. Alexandra vyhrá, ak sa jej podarilo vytvoriť riadok pozostávajúca iba zo zelených tehiel; inak vyhrá Benjamín. Nájdite všetky dvojice $(m, n)$, pre ktoré vie Alexandra zaručene vyhrať, nech hrá Benjamín ľubovoľne.}
\podpis{...}

{%%%%%   vyberko, den 1, priklad 2
 Je daný ostrouhlý trojuholník $ABC$ s~pätami výšok $D$, $E$, $F$ po rade z~vrcholov $A$, $B$, $C$. Označme $\omega_B$ a~$\omega_C$ kružnice vpísané trojuholníkom $BDF$ a~$CDE$. Ďalej označme $M$ a~$N$ po rade dotykové body $\omega_B$ a~$\omega_C$ s~úsečkami~$DF$ a~$DE$. Priamka $MN$ pretína kružnice $\omega_B$ a~$\omega_C$ po rade v~bodoch $P \ne M$ a~$Q \ne N$. Dokážte, že $|MP|=|NQ|$.}
\podpis{...}

{%%%%%   vyberko, den 1, priklad 3
 Nech $n \ge 2$ je kladné celé číslo a~$a_1,\ldots,a_n$ sú reálne čísla so súčtom~0. Definujme množinu~$A$ nasledovne:
$$
...
{%%%%%   vyberko, den 2, priklad 1
 Pre všetky celé čísla $n>1$ nájdite všetky nekonštantné polynómy~$P(x)$ s~reálnymi koeficientami, ktoré pre všetky reálne čísla~$x$ spĺňajú
$$
...
{%%%%%   vyberko, den 2, priklad 2
 Nájdite všetky trojice $(a,b,c)$ kladných celých čísel spĺňajúcich $a^3+b^3+c^3=(abc)^2$.}
\podpis{...}

{%%%%%   vyberko, den 2, priklad 3
 Body $X$ a~$Y$ ležia vnútri strán $AB$ a~$AC$ ostrouhlého trojuholníka $ABC$ tak, že obraz priamky $BC$ v~osovej súmernosti podľa priamky $XY$ je dotyčnica ku kružnici~$\omega$ opísanej trojuholníku~$AXY$. Označme~$O$ stred kružnice opísanej trojuholníku~$ABC$. Dokážte, že kružnica opísaná trojuholníku~$BCO$ sa dotýka~$\omega$.}
\podpis{...}

{%%%%%   vyberko, den 3, priklad 1
 Nech $f : {\Bbb Z} \to {\Bbb Z}$ je funkcia taká, že pre všetky celé čísla~$x$ a~$y$ platí
$$
f(f(x)-y)=f(y)-f(f(x)).
$$
...
{%%%%%   vyberko, den 4, priklad 1
 Nájdite všetky možné hodnoty výrazu $a^3+b^3+c^3-3abc$, kde $a,b,c$ sú nezáporné celé čísla.}
\podpis{...}

{%%%%%   vyberko, den 4, priklad 2
 Nech $u_1,\ldots,u_{2020}$ sú reálne čísla spĺňajúce
$$
\eqalign{
...
{%%%%%   vyberko, den 4, priklad 3
 Na stole leží~69 prázdnych krabíc $K_1,\ldots,K_{69}$. Máme k~dispozícií neobme\-dze\-né množstvo guľôčok. Je dané celé číslo~$n$. Anastázia a~Bartolomej hrajú nasledovnú hru: V~prvom kole Anastázia vezme~$n$ guľôčok a~rozdelí ich do 69~krabíc podľa svojho priania. Každé ďalšie kolo pozostáva z~nasledovných krokov:
...
{%%%%%   vyberko, den 5, priklad 1
 Je daný trojuholník~$ABC$. Kružnica~$\Gamma$ prechádzajúca bodom~$A$ pretína úsečky $AB$ a~$AC$ po rade v~jej vnútorných bodoch~$D$ a~$E$, a~tiež pretína úsečku $BC$ v~jej vnútorných bodoch~$F$ a~$G$ tak, že~$F$ leží medzi~$B$ a~$G$. Dotyčnica ku kružnici opísanej $BDF$ v~$F$ a~dotyčnica ku kružnici opísanej $CEG$ v~$G$ sa pretínajú v~bode~$T$. Predpokladajme, že $A \ne T$. Dokážte, že $AT \parallel BC$.}
\podpis{...}

{%%%%%   vyberko, den 5, priklad 2
 Je daná množina~$n$ cukríkov, pričom každý váži aspoň~1~gram, a ich celková váha je~$2n$ gramov. Dokážte, že pre každé reálne číslo~$r$ spĺňajúce $0 \leq r \leq 2n-2$ vieme vybrať cukríky, ktorých celková váha je aspoň~$r$ gramov, avšak najviac $r+2$ gramov.}
\podpis{...}

{%%%%%   vyberko, den 5, priklad 3
 Nech ${\Bbb Z}_{>0}$ označuje množinu kladných celých čísel. Je daná kladná celočíselná konštanta~$K$. Nájdite všetky funkcie $f\colon {\Bbb Z}_{>0} \rightarrow {\Bbb Z}_{>0}$ také, že pre všetky kladné celé čísla $a$, $b$ spĺňajúce $a+b>K$ platí
$$
\abovedisplayskip0pt
a+f(b) \mid a^2+bf(a).
\belowdisplayskip0pt
