...
{%%%%%   CPSJ, priklad t1
Je daný pravouhlý trojuholník $ABC$ s preponou $AB$. Označme $D$ pätu výšky z~vrcholu $C$. Nech $Q$, $R$ a $P$ sú po rade stredy úsečiek $AD$, $BD$ a $CD$. Dokážte, že platí
$$
...
{%%%%%   CPSJ, priklad t2
Nájdite najväčšie možné celé číslo $d$, ktoré súčasne delí tri trojciferné čísla $\overline{abc}$, $\overline{bca}$ a~$\overline{cab}$, kde $a$,~$b$~ a~$c$ sú vhodné nenulové a navzájom rôzne číslice.
}
\podpis{Jaromír Šimša}

{%%%%%   CPSJ, priklad t3
V rovine je daný istý počet priamok tak, že každá pretína práve 15 iných. Koľko priamok je daných? Charakterizujte všetky možné konfigurácie a~zdôvodnite, že neexistujú žiadne iné.
}
\podpis{Barbara Roszkowska-Lech}

{%%%%%   CPSJ, priklad t4
Istý počet dlaždíc zhodných s tou, ktorá je znázornená na obrázku, treba umiestniť do štvorca o~rozmeroch $11\times 11$ rozdeleného na polia, ktoré sú jednotkovými štvorcami, takým spôsobom, aby každá dlaždica pokrývala presne $6$ polí, žiadna nepresahovala mimo štvorca a žiadne dve nepokrývali to isté pole.\insp{5cpsj-prop.30}
\ite{a)} Určte najväčší možný počet dlaždíc, ktoré možno umiestniť do štvorca opísaným spôsobom.
\ite{b)} Nájdite všetky polia, ktoré musia byť pokryté pri každom pokrytí s~použitím maximálneho počtu dlaždíc.\endgraf
%
}
