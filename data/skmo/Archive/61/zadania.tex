...
{%%%%%   CPSJ, priklad t3
Dokážte, že ak $n$ je kladné celé číslo, tak číslo $2(n^2 + 1) - n$ nie je
štvorcom celého čísla.}
\podpis{Poľsko}

{%%%%%   CPSJ, priklad t4
Daný je kosoštvorec $ABCD$, v ktorom $\uhol BAD = 60\st$. Bod $P$ leží vnútri kosoštvorca, pričom
sú splnené rovnosti $BP = 1$, $DP = 2$, $CP = 3$. Určte dĺžku úsečky $AP$.}
\podpis{Poľsko}

{%%%%%   CPSJ, priklad t5
Určte všetky trojice $(a,k,m)$ kladných celých čísel, ktoré vyhovujú rovnici
$$k+a^k=m+2a^m.$$
}
\podpis{Poľsko}

{%%%%%   CPSJ, priklad t6
Šachovnicovú dosku $8\times8$ máme pokryť pomocou rovinných útvarov rovnakých
ako na \obr{} (každý z útvarov môžeme otočiť o~$90\st$) tak, že sa žiadne
dva neprekrývajú ani nepresahujú cez okraj šachovnice. Určte, aký najväčší
možný počet polí tejto šachovnice môžeme uvedeným spôsobom pokryť.
\insp{61-cpsj-t6}%
}
\podpis{Poľsko}
