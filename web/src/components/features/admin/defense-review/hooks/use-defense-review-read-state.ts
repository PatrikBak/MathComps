...
/**
 * What one call to mark a conversation read or unread says.
 */
type ReadStateChange = {
  /** The conversation. */
  sessionId: string
  /** True to stamp it as read, false to leave it unread. */
  read: boolean
}
...
export function useDefenseReviewReadState(): UseDefenseReviewReadStateResult {
  ...
  const { mutate } = useOptimisticMutation<void, ReadStateChange, ReadStateSnapshot | null>({
    apiFn: (apiCall, { sessionId, read }) => setDefenseReviewReadState(apiCall, sessionId, read),
    onMutate: ({ sessionId, read }) => {
      ...
      patchCachedQueueConversation(queryClient, sessionId, (conversation) => {
        ...
        // Read as of now with nothing left in it, or back to every reply standing unread
        written.value = read
          ? { readAt: new Date().toISOString(), unreadTurnCount: 0 }
          : { readAt: null, unreadTurnCount: conversation.turnCount }
  ...
  >({
    ...
    onMutate: ({ sessionIds, read }) => {
      ...
      patchCachedQueueConversations(queryClient, sessionIds, (conversation) => {
        ...
        // Read as of now with nothing left in it, or back to every reply standing unread
        return read
          ? { ...conversation, readAt, unreadTurnCount: 0 }
          : { ...conversation, readAt: null, unreadTurnCount: conversation.turnCount }
      })
  ...
  const markRead = useCallback((sessionId: string) => mutate({ sessionId, read: true }), [mutate])

  // Leaves a conversation unread
  const markUnread = useCallback(
    (sessionId: string) => mutate({ sessionId, read: false }),
    [mutate]
  )
  ...
  return { markRead, markUnread, markMany }
}
