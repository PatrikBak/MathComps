...
public static class RegistryLinkValidator
{
    ...
    public static IReadOnlyList<VerdictError> Check(IMetadataLocalizationService metadata, ManifestMeta meta) =>
        // Ask the registry for the competition's gaps and shape each one as a folder-level preflight error.
        [.. metadata.ValidateTaxonomyRegistration(meta.CompetitionPath).Select(ToVerdictError)];
    ...
    /// some locales), and a competition that turns out to carry others below it.
    /// </summary>
    ...
    public static VerdictError ToVerdictError(TaxonomyRegistryIssue issue)
    {
        ...
        if (issue.CarriesNestedCompetitions)
            gaps.Add("competitions nested below it, so a draft names one of those instead");

        // Compose a single readable message, e.g. "competition 'csmo-a-iii' has no localized name in cs, en".
        var message = $"competition '{issue.Path}' has {gaps.ToJoinedString("; ")}";
