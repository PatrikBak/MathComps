...
Tags a bulk-import draft in place with Gemini, writing the chosen tag slugs into each problem's `pN.yaml` sidecar. One command: `tag-draft`.
...
For every problem whose `pN.yaml` has **no** `tags:` key, `tag-draft` runs four Gemini passes over the draft's original-language body (the language `_meta.yaml` marks):
...
- **Skip rule** — `tag-draft` skips any problem whose `pN.yaml` already has a `tags:` key (a populated list *or* an empty one — empty means "decided: no tags"). A re-run only fills in problems that still have no key, so hand-edits are never overwritten and a partial run is resumable.
...
```bash
dotnet run --project backend/src/MathComps.Cli.Tagging -- tag-draft ./my-draft
```
...
```
author draft → tag-draft → validate → apply (local) → eyeball on site → fix yaml → apply (prod)
```
