...
```bash
dotnet run --project backend/src/Tools/MathComps.Cli.BulkImport -- validate ./my-draft
dotnet run --project backend/src/Tools/MathComps.Cli.BulkImport -- validate ./my-draft --json
```
...
```bash
dotnet run --project backend/src/Tools/MathComps.Cli.BulkImport -- apply ./my-draft
```
...
- **Cloudflare R2** (`apply` only) — image uploads need the `CloudflareR2` settings:
  ...
  ```bash
  cd backend/src/Tools/MathComps.Cli.BulkImport
