...
- **Prefer the `_common.asy` helpers over raw asy primitives.** If you find yourself writing `draw(A -- B, pen)`, use `Draw(A, B, pen)` instead — the wrapper keeps line caps, default widths, and style handling consistent across figures. Same for `Circle`, `LabeledDot`, etc.
