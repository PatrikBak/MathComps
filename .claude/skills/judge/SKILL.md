...
- The examiner's own brief — `backend/src/MathComps.Cli.Examiner/Prompts/generate.txt` (what it was told to do), plus the two guards `math-check.txt` and `leak-check.txt`. You grade the examiner against its brief, so read it before judging, not only when proposing fixes.
...
- **Concrete prompt-fix ideas** — the payload. Read the current templates first — `backend/src/MathComps.Cli.Examiner/Prompts/{generate,math-check,leak-check}.txt` and the per-step models in `backend/src/MathComps.Cli.Examiner/appsettings.json` — then name what in them would plausibly fix each failure. This is the loop that tunes the engine.
