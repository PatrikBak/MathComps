...
9. **Display equations must stay narrow enough for a phone.** Keep each typeset line under ~75% of `\hsize`. Split at an `=`, with the continuation line starting `= …`, using `\gather … \endgather`; when the display carries an `\eqno`, use `\eqalign{…}` instead, since `\gather` is AMS-TeX's own display environment and wants `\tag`, not `\eqno`.
